...
const OPENAI_MODELS: readonly ProviderModelDefinition[] = [
  { id: "gpt-5.4", label: "GPT-5.4" },
  { id: "gpt-5.4-mini", label: "GPT-5.4 Mini" },
  { id: "gpt-5.3-codex", label: "GPT-5.3 Codex" }
];

const ANTHROPIC_MODELS: readonly ProviderModelDefinition[] = [
  { id: "claude-sonnet-4-5", label: "Claude Sonnet 4.5" },
  { id: "claude-opus-4-1", label: "Claude Opus 4.1" },
  { id: "claude-haiku-3-5", label: "Claude Haiku 3.5" }
];

const MISTRAL_MODELS: readonly ProviderModelDefinition[] = [
  { id: "mistral-large-latest", label: "mistral-large" },
  { id: "mistral-medium-latest", label: "mistral-medium" },
  { id: "mistral-small-latest", label: "mistral-small" }
];

const FALLBACK_LOCAL_MODELS: readonly ProviderModelDefinition[] = [
...
const PROVIDER_DEFINITIONS: Record<ModelProvider, ProviderDefinition> = {
  local: {
    name: "local",
    label: "Local",
    ...
    createConfig(settings) {
      ...
      return {
        ...
        transport: "ollama",
        baseUrl:
          normalizeOptionalValue(settings?.baseUrl) ?? DEFAULT_OLLAMA_BASE_URL
      };
    },
    createClient(config) {
      if (config.provider !== "local") {
        throw new Error(
          `Local provider cannot create client for ${config.provider}.`
        );
      }
  ...
  openai: {
    ...
    createConfig(settings) {
      ...
      return {
        ...
        apiKey,
        baseUrl:
          normalizeOptionalValue(settings?.baseUrl) ?? DEFAULT_OPENAI_BASE_URL
      };
    },
    createClient(config) {
      if (config.provider !== "openai") {
        throw new Error(
          `OpenAI provider cannot create client for ${config.provider}.`
        );
      }
  ...
  anthropic: {
    ...
    createConfig(settings) {
      ...
      return {
        provider: "anthropic",
        model:
          normalizeOptionalValue(settings?.model) ?? ANTHROPIC_MODELS[0].id,
        apiKey,
        baseUrl:
          normalizeOptionalValue(settings?.baseUrl) ??
    ...
    createClient(config) {
      if (config.provider !== "anthropic") {
        throw new Error(
          `Anthropic provider cannot create client for ${config.provider}.`
        );
      }
  ...
  mistral: {
    ...
    createConfig(settings) {
      ...
      return {
        provider: "mistral",
        model:
          normalizeOptionalValue(settings?.model) ?? MISTRAL_MODELS[0].id,
        apiKey,
        baseUrl:
          normalizeOptionalValue(settings?.baseUrl) ?? DEFAULT_MISTRAL_BASE_URL
      };
    },
    createClient(config) {
      if (config.provider !== "mistral") {
        throw new Error(
          `Mistral provider cannot create client for ${config.provider}.`
        );
      }
...
export function getProviderDefinition(provider: ModelProvider): ProviderDefinition {
  return PROVIDER_DEFINITIONS[provider];
}

export function listProviderDefinitions(): readonly ProviderSummary[] {
