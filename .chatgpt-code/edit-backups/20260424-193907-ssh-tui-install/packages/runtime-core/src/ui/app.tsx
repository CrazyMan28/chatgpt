...
import type { RegisteredTool, ToolRegistry } from "../tools/tool-registry.js";
...
export function App({
  ...
  const refreshRemoteAgents = async (): Promise<
    readonly {
      currentTask?: string;
      host: string;
      id: string;
    ...
    try {
      switch (command.type) {
        ...
        case "ssh_add":
          ...
          if (!disposedRef.current) {
            startTransition(() => {
              appendSystemMessage(`Added SSH worker "${command.name}" (${command.host}).`);
            });
          }
