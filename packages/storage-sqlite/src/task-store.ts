...
export class SqliteTaskStore implements TaskStore {
  ...
  async saveTask(task: BackgroundTaskRecord): Promise<BackgroundTaskRecord> {
    ...
    this.database.run(
      ...
      normalized.lastResultSummary ?? null,
      normalized.lastRunAt ?? null,
      normalized.autoState ? JSON.stringify(normalized.autoState) : null,
      JSON.stringify(buildTaskData(normalized))
    );
...
function parseTaskRow(row: TaskRow): BackgroundTaskRecord {
  ...
  return normalizeTask({
    ...
    autoState: row.auto_state_json
      ? (JSON.parse(row.auto_state_json) as BackgroundTaskRecord["autoState"])
      : undefined,
...
function parseTaskData(
  ...
  title: string;
  transcriptPrompt?: string;
  validationStatus: BackgroundTaskRecord["validationStatus"];
  watcherIds: string[];
} {
  if (!value) {
    return {
    ...
    return {
      ...
      blockedReason:
        typeof parsed.blockedReason === "string"
          ...
          : undefined,
      currentStep:
        typeof parsed.currentStep === "string" ? parsed.currentStep : undefined,
      ...
      resumePrompt:
        typeof parsed.resumePrompt === "string" ? parsed.resumePrompt : undefined,
      retries:
        typeof parsed.retries === "number" ? Math.max(0, Math.floor(parsed.retries)) : 0,
      ...
      title:
        typeof parsed.title === "string" && parsed.title.trim().length > 0
          ...
          : "Interactive task",
      transcriptPrompt:
        typeof parsed.transcriptPrompt === "string"
...
function readRequestedAction(
  value: unknown
): BackgroundTaskRecord["requestedAction"] | undefined {
  return value === "pause" ||
    value === "resume" ||
    value === "retry" ||
    value === "stop" ||
