...
export class LinuxDesktopDriver {
  async getEnvironment(): Promise<DesktopEnvironment> {
    return {
  ...
  async screenshot(): Promise<ScreenshotResult> {
    ...
    for (const attempt of attempts) {
      ...
      if (result.code !== 0) {
        errors.push(
          `${attempt.command}: exit ${result.code}${result.stderr ? `: ${result.stderr}` : ""}`
        );
        continue;
      }
  ...
  async listWindows(): Promise<WindowInfo[]> {
    ...
    if (result.code !== 0) {
      throw new Error(
        `Window listing failed with wmctrl: ${result.stderr || "unknown error"}`
      );
    }
    ...
    return result.stdout
      ...
      .filter((entry): entry is WindowInfo => entry !== undefined);
  }

  private async listWindowsFromAccessibility(): Promise<WindowInfo[]> {
  ...
  async activeWindow(): Promise<WindowInfo | undefined> {
    ...
    if (activeId && (await commandExists("xdotool"))) {
        const titleResult = await runCommand(
          "xdotool",
          ["getwindowname", String(Number.parseInt(activeId, 16))],
          { timeoutMs: 5_000 }
        ).catch(() => undefined);
    ...
    if (activeId && (await commandExists("wmctrl"))) {
      ...
      if (result?.code === 0) {
        return result.stdout
          .split(/\r?\n/)
          .map((line) => parseWmctrlLine(line, activeId))
          .find((window): window is WindowInfo => window?.isActive === true);
      }
    }
  ...
  async focusWindow(windowId: string): Promise<void> {
    ...
    throw new Error(
      "Window focus requires wmctrl on this desktop. Install it with: sudo dnf install wmctrl"
    );
  }
  ...
  async closeWindow(windowId: string): Promise<void> {
    ...
    throw new Error(
      "Window close requires wmctrl on this desktop. Install it with: sudo dnf install wmctrl"
    );
  }
  ...
  async openApp(app: string): Promise<{ command: string; app: string }> {
    ...
    for (const candidate of candidates) {
      if (candidate.desktopId && (await commandExists("gtk-launch"))) {
        ...
        if (result) {
          errors.push(
            `gtk-launch ${launchId}: ${result.stderr || `exit ${result.code}`}`
          );
        }
      }
  ...
  async openUrl(url: string): Promise<{ command: string; url: string }> {
  ...
  async accessibilityClick(elementId: string): Promise<{
    bounds?: Bounds;
    message?: string;
    ok: boolean;
  }> {
  ...
  async accessibilityFocus(elementId: string): Promise<{
    bounds?: Bounds;
    message?: string;
    ok: boolean;
  }> {
  ...
  async mouseMove(x: number, y: number): Promise<void> {
    ...
    throw new Error(
      "Raw mouse movement requires xdotool under X11. Wayland desktops usually block synthetic pointer input unless ydotool/dotool is installed and configured."
    );
  }
  ...
  async mouseClick(x: number, y: number, button = 1): Promise<void> {
    ...
    throw new Error(
      "Raw mouse click requires xdotool under X11. On Wayland, prefer accessibility actions or configure a trusted input tool such as ydotool."
    );
  }
  ...
  async mouseScroll(clicks: number): Promise<void> {
    if (await commandExists("xdotool")) {
  ...
  async keyboardType(text: string): Promise<void> {
    ...
    throw new Error(
      "Keyboard typing requires xdotool on X11 or wtype on Wayland. Install one with: sudo dnf install xdotool wtype"
    );
  }
  ...
  async keyboardHotkey(keys: readonly string[]): Promise<void> {
    if (keys.length === 0) {
  ...
  async clipboardRead(): Promise<string> {
    ...
    if (!command) {
      throw new Error(
        "Clipboard read requires wl-paste, xclip, or xsel. Install with: sudo dnf install wl-clipboard xclip xsel"
      );
    }
  ...
  async clipboardWrite(text: string): Promise<void> {
    ...
    if (!command) {
      throw new Error(
        "Clipboard write requires wl-copy, xclip, or xsel. Install with: sudo dnf install wl-clipboard xclip xsel"
      );
    }
...
function parseWmctrlLine(line: string, activeId?: string): WindowInfo | undefined {
  const match = /^(0x[0-9a-fA-F]+)\s+\S+\s+(-?\d+)\s+(-?\d+)\s+(-?\d+)\s+(-?\d+)\s+(\d+)\s+\S+\s+(.*)$/.exec(
    line.trim()
  );
...
async function readActiveWindowId(): Promise<string | undefined> {
  if (!(await commandExists("xdotool"))) {
    return undefined;
  }
...
async function listInstalledDesktopApps(): Promise<string[]> {
  const dirs = [
...
async function resolveAppCandidates(app: string): Promise<AppCandidate[]> {
  const normalized = app.trim().toLowerCase();
  const fallback = app.trim();
  const aliases: Record<string, AppCandidate[]> = {
    browser: [
...
function vscodeCandidates(): AppCandidate[] {
  return [
    { command: "code" },
...
async function findDesktopCandidates(app: string): Promise<AppCandidate[]> {
  ...
  for (const dir of dirs) {
    ...
    for (const file of files) {
      ...
      if (
        normalizeAppName(id).includes(normalized)
      ) {
...
async function desktopIdExists(desktopId: string): Promise<boolean> {
...
async function suggestInstalledApps(app: string): Promise<string[]> {
  const normalized = normalizeAppName(app);
  const installed = await listInstalledDesktopApps().catch(() => []);
...
function dedupeAppCandidates(
  candidates: readonly (AppCandidate | undefined)[]
): AppCandidate[] {
...
function formatAppCandidates(candidates: readonly AppCandidate[]): string {
  return [
...
async function runAccessibilityScript(input: {
  ...
  const result = await runCommand(
    "python3",
    ["-c", PYATSPI_SCRIPT, JSON.stringify(input)],
    { timeoutMs: 12_000 }
  ).catch((error: unknown) => ({
    code: 1,
  ...
  if (result.code !== 0) {
    return {
      ...
      generatedAt: Date.now(),
      summary:
        "Accessibility unavailable. Install python3-pyatspi and ensure the desktop accessibility bridge is enabled."
    };
  }
...
async function runAccessibilityAction(
  action: "click" | "focus",
  elementId: string
): Promise<{ bounds?: Bounds; message?: string; ok: boolean }> {
  if (!(await commandExists("python3"))) {
    return {
...
function parseElement(value: unknown): DesktopElement | undefined {
  ...
  const children = Array.isArray(value.children)
    ? value.children
        .map(parseElement)
        .filter((entry): entry is DesktopElement => entry !== undefined)
    : undefined;
  ...
  return {
    ...
    states: Array.isArray(value.states)
      ? value.states.filter((entry): entry is string => typeof entry === "string")
      : [],
...
function countElements(element: DesktopElement): number {
  return 1 + (element.children ?? []).reduce((sum, child) => sum + countElements(child), 0);
}

function formatElementLabel(element: DesktopElement): string {
