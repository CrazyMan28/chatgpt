import type { DesktopActionResult } from "./types.js";
import {
  ...
  try {
    switch (request.method) {
      ...
      case "tools/call": {
        ...
        const toolCallResult = await callTool(name, args).catch((error: unknown) =>
          result(
            {
              ok: false,
...
async function callTool(
  ...
  switch (name) {
    ...
    case "screen.describe":
      return result(
        await controller.describeScreen({
          prompt: readOptionalString(input.prompt)
        })
      );
    case "screen.locate":
      return result(
        await controller.locateOnScreen({
          target: readString(input.target, "target")
        })
      );
    case "screen.compare":
      return result(
        await controller.compareScreens({
    ...
    case "screen.watch":
      return result(
        await controller.startWatch({
          intervalMs:
            typeof input.intervalMs === "number" ? readNumber(input.intervalMs, "intervalMs") : undefined,
    ...
    case "accessibility.find":
      return result(compactAccessibilityFindResult(
        await controller.accessibilityFind({
    ...
    case "accessibility.type":
      return actionResult(
        await controller.accessibilityType(
      ...
      return actionResult(
        await controller.act({
          keys: Array.isArray(input.keys)
            ? input.keys.filter((entry): entry is string => typeof entry === "string")
            : [],
          type: "keypress"
        })
      );
    ...
    default:
      return result(
        ...
        `Unknown desktop-control tool: ${name}`,
        true
      );
  }
}
...
function actionResult(action: DesktopActionResult): Record<string, unknown> {
...
function compactObservation(observation: Awaited<ReturnType<typeof controller.observe>>): Record<string, unknown> {
  return {
    ...observation,
...
function compactAccessibilityFindResult(input: Awaited<ReturnType<typeof controller.accessibilityFind>>): Record<string, unknown> {
  return {
...
function sendResult(id: JsonRpcId, value: unknown): void {
...
export async function handleDesktopCommand(input: {
  ...
  switch (input.command) {
    ...
    case "vision.provider": {
      const config = await controller.setVisionProvider(
        readVisionProvider(input.args?.provider)
      );
