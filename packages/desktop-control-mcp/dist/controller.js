import { createDefaultDesktopState, loadDesktopState, parseMode, parseVisionProvider, saveDesktopState } from "./state.js";
import { LinuxDesktopDriver } from "./linux.js";
import { createVisionProvider } from "./vision.js";
import { delay, hashFile, isRecord, readBoolean, readNumber, readOptionalString, readString, readStringArray, truncate } from "./utils.js";
const WATCH_INTERVAL_MS = 5_000;
export class DesktopController {
    options;
    driver = new LinuxDesktopDriver();
    now;
    statePath;
    lastAction;
    lastObservation;
    pendingApproval;
    watcherHashes = new Map();
    ...
    async setVisionProvider(provider) {
        const state = await this.loadState();
    ...
    async status() {
        ...
        return {
            activeWatcher: [...this.watchers.values()].find((watcher) => ["active", "healthy", "stalled"].includes(watcher.state)),
    ...
    async describeScreen(input = {}) {
        ...
        return createVisionProvider(config, this.options.getVisionAuth?.()).describeScreen(screenshot, input.prompt, input.context);
    }
    async locateOnScreen(input) {
        const config = await this.getConfig();
        const screenshot = await this.screenshot();
        return createVisionProvider(config, this.options.getVisionAuth?.()).locateElement(screenshot, input.target, input.context);
    }
    async compareScreens(input) {
        const config = await this.getConfig();
        return createVisionProvider(config, this.options.getVisionAuth?.()).compareScreens(input.beforePath, input.afterPath, input.prompt);
    }
    async observe(input = {}) {
        const environment = await this.driver.getEnvironment();
        ...
        const accessibility = includeAccessibility
            ...
            : undefined;
        const screenshot = input.screenshot === false
            ? undefined
    ...
    async accessibilityType(elementId, text) {
        return this.act({
            target: {
                elementId,
                kind: "accessibility"
        ...
        try {
            if (/open .*text editor|text editor/.test(normalized)) {
                ...
                if (!actions[actions.length - 1]?.ok) {
                    await this.finishWatch(watcher.id, "stalled", actions[actions.length - 1]?.reason);
                    return {
                        actions,
                        done: false,
            ...
            await this.finishWatch(watcher.id, "failed", error instanceof Error ? error.message : String(error));
            throw error;
        }
    }
    ...
    async authorizedPrimitive(action, risk, operation, summary) {
        ...
        switch (action.type) {
            ...
            case "focus":
                ...
                if (action.target?.kind === "app" && action.target.app) {
                    ...
                    const match = windows.find((window) => window.title.toLowerCase().includes(action.target?.app?.toLowerCase() ?? ""));
                    if (!match) {
            ...
            case "click":
                if (action.target?.kind === "accessibility" && action.target.elementId) {
                    ...
                    if (result.bounds) {
                        await this.driver.mouseClick(Math.round(result.bounds.x + result.bounds.width / 2), Math.round(result.bounds.y + result.bounds.height / 2));
                        return {
    ...
    async authorizeAction(action, risk, summary = summarizeAction(action)) {
        if (risk === "safe") {
            return {
                approved: true
            };
    ...
    async checkAppPolicy(config, action) {
        ...
        if (config.allowedApps.length > 0 && action.type !== "open_app") {
            if (!targetApp) {
                return {
                    approved: false,
                    reason: "Desktop app allowlist is active, but the active app/window could not be identified."
                };
            }
    ...
    async tickWatcher(id) {
        ...
        this.watchers.set(id, {
            ...
            state: now - (progressed ? now : watcher.lastProgressTime) > 60_000
                ? "stalled"
                : "healthy"
        });
    }
    async finishWatch(id, state, stopReason) {
...
export function parseAction(input) {
    ...
    const target = isRecord(input.target)
        ? {
            ...
            kind: readString(input.target.kind, "target.kind"),
...
export function readDesktopMode(value) {
...
export function readVisionProvider(value) {
...
function createCapabilities(environment, accessibility) {
    ...
    const hasWindowBackend = ["wmctrl", "xdotool", "qdbus"].some((command) => commands.has(command));
    const hasAppBackend = ["gtk-launch", "xdg-open"].some((command) => commands.has(command));
    const hasInputBackend = ["xdotool", "wtype"].some((command) => commands.has(command));
    return {
        accessibility: {
...
export function formatDesktopStatus(status) {
    const activeWindow = status.lastObservation?.activeWindow?.title ??
        status.lastObservation?.screenshot?.activeWindow?.title ??
        "-";
    const activeApp = status.lastObservation?.activeWindow?.app ??
...
export function summarizeAccessibilityTree(tree, full = false) {
    if (!tree.available) {
        return tree.summary;
    }
    if (!tree.root) {
    ...
    const nodes = flattenElements(tree.root)
        ...
        const label = [element.id, element.role, element.name]
            .filter(Boolean)
            .join("  ");
        const actions = element.actions.length > 0 ? ` actions=${element.actions.join(",")}` : "";
...
function isApprovalBlock(result) {
    return (result.approvalId !== undefined &&
...
export function readToolBoolean(input, key, fallback) {
    return readBoolean(input[key], fallback);
}
export function readToolNumber(input, key, fallback) {
...
function classifyActionRisk(action) {
    ...
    const text = [
        ...
        .join(" ")
        .toLowerCase();
    if (/\b(send|submit|purchase|buy|bank|wire|transfer|password|passwd|delete|remove|install|sudo|token|secret|credential|scan|attack|exploit|remote device)\b/.test(text)) {
        return "dangerous";
    }
    return "medium";
}
...
function matchesElement(element, input) {
...
function extractTextToType(task) {
