...
export function buildResponseModeContextMessage(mode) {
    return {
        role: "system",
...
export function formatResponseModeLabel(mode) {
    switch (mode) {
        case "normal":
            return "Normal";
        case "plan":
...
export function readResponseModeFromMessages(messages) {
    const modeMessage = messages.find((message) => message.role === "system" &&
        message.content.startsWith(MODE_CONTEXT_PREFIX));
    if (!modeMessage || modeMessage.role !== "system") {
        return undefined;
    }
    const match = modeMessage.content.match(/^Response mode:\s+(normal|plan|ultra)\b/i);
    if (!match) {
        return undefined;
    }
...
//# sourceMappingURL=response-mode.js.map