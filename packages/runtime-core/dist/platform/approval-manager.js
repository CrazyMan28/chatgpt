...
export function createApprovalManager(store, defaultPolicy) {
    ...
    return {
        ...
        async grant(scope, options) {
            ...
            const grant = {
                ...
                scope,
                sessionId: scope === "session" || scope === "task" ? activeSessionId : undefined,
        ...
        setActiveSession(sessionId) {
            ...
            if (policy.blanketGrant?.scope === "session" &&
        ...
        setActiveTask(taskId) {
            activeTaskId = taskId;
            if (policy.blanketGrant?.scope === "task" &&
...
export function readScopeForApproval(context) {
    return context.scope;
}
function normalizePolicy(policy) {
    const normalizedPreset = normalizePresetName(policy.preset ?? inferPreset(policy));
    return {
        ...presetToPolicy(normalizedPreset, policy),
        ...policy
    };
}
function inferPreset(policy) {
    if (policy.preset) {
...
function presetToPolicy(preset, currentPolicy) {
    const common = {
        fullMachineRequiresApproval: currentPolicy?.fullMachineRequiresApproval ?? true,
        homeWriteRequiresApproval: currentPolicy?.homeWriteRequiresApproval ?? true,
        remoteHostRequiresApproval: currentPolicy?.remoteHostRequiresApproval ?? true,
        trustedWorkspaceRoots: currentPolicy?.trustedWorkspaceRoots,
        blanketGrant: preset === "auto-safe" ||
...
async function approvePendingForGrant(store, grant) {
    ...
    for (const request of pending) {
        ...
        if ((grant.scope === "session" || grant.scope === "task") &&
...
async function findMatchingPendingRequest(store, input) {
    const pending = await store.listPending();
    return pending.find((request) => {
        return (request.metadata.fingerprint === input.fingerprint &&
...
function isAutoApproved(input) {
    ...
    if (input.scope === "remote-host" &&
        input.policy.remoteHostRequiresApproval !== false) {
        return false;
    }
    if (input.scope === "full-machine" &&
