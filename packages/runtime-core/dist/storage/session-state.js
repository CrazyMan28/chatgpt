...
export function getRecentSessionHistory(history, options) {
    ...
    const startIndex = normalizeHistoryWindowStart(history, Math.max(0, history.length - limit));
...
function normalizeHistoryWindowStart(history, startIndex) {
...
function sanitizeRecentHistoryWindow(messages) {
    const sanitized = [];
