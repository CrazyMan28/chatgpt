...
export function buildTaskPlanPrompt(taskPrompt) {
    return [
...
export function buildPlanRevisionPrompt(currentPlan, feedback, originalTaskPrompt) {
    return [
...
export function detectPlanClarificationState(taskPrompt) {
    ...
    const refersToUnknownTarget = /\b(it|this|that|thing|stuff|shit)\b/.test(lowered) &&
        !/\b(file|screen|page|component|module|api|plan|build|session|memory|mcp|provider|model)\b/.test(lowered);
    const actionButNoTarget = /\b(fix|change|update|improve|implement|build|refactor|rewrite)\b/.test(lowered) &&
        !/\b(file|screen|page|component|module|api|plan|build|session|memory|mcp|provider|model|readme|docs|test|tests)\b/.test(lowered);
    if (wordCount < 4) {
...
export function mergeClarificationIntoPlanPrompt(clarificationState, answer) {
...
//# sourceMappingURL=plan-workflow.js.map