...
import { createEnabledMcpServerConfigs, readAppConfigFile, writeAppConfigFile } from "../config/app-config-file.js";
export function createMcpManager(options) {
    ...
    return {
        ...
        async getServer(name) {
            return listServersInternal().then((servers) => servers.find((server) => server.name === name));
        },
        async listServers() {
        ...
        async refresh() {
            ...
            const reloadSummary = await toolRegistry.reload(createEnabledMcpServerConfigs(resolved.file, resolved.configDirectory));
            return {
    ...
    async function listServersInternal() {
        const resolved = readAppConfigFile(env, cwd);
    ...
    async function mutateConfig(mutator) {
        ...
        const reloadSummary = await toolRegistry.reload(createEnabledMcpServerConfigs(result.file, resolved.configDirectory));
        const server = result.name === undefined
            ? undefined
            : {
                name: result.name,
...
export function formatMcpServerList(servers) {
    if (servers.length === 0) {
    ...
    for (const server of servers) {
        lines.push(`${padCell(server.name, 14)} ${padCell(server.type, 8)} ${server.status}`);
    }
    return lines.join("\n");
}
export function formatMcpReloadSummary(actionLabel, result) {
    const loadedSummary = result.reloadSummary.loadedServerNames.length > 0
        ? `Loaded: ${result.reloadSummary.loadedServerNames.join(", ")}.`
        : "Loaded: none.";
    const failedSummary = result.reloadSummary.failedServerNames.length > 0
...
function validateServerName(name) {
    if (!/^[A-Za-z0-9._-]+$/.test(name.trim())) {
        throw new Error("MCP server names must use only letters, numbers, dot, dash, or underscore.");
    }
}
async function normalizeDraft(draft, configDirectory) {
...
function validateExecutable(command, cwd, configDirectory) {
    if (command.includes("/") || command.startsWith(".")) {
        const resolvedPath = resolve(cwd.length > 0 ? resolve(configDirectory, cwd) : configDirectory, command);
...
function toDefinition(draft) {
    return {
        ...
        headers: draft.headers && Object.keys(draft.headers).length > 0
            ? { ...draft.headers }
            : {},
...
function fromDefinition(name, definition) {
    return {
