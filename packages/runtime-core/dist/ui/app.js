...
import { formatRetryDelay, isRateLimitError, readErrorMessage, runWithRateLimitRecovery } from "../agent/rate-limit-recovery.js";
import { buildPromptContextMessages, buildAgentPromptContext, mergeAgentTurnHistory } from "../memory/prompt-builder.js";
import { detectFleetDelegationIntent, inferFleetRoleFromPrompt } from "../fleet/fleet-manager.js";
...
import { buildPlanConfirmationMessage, buildPlanExecutionPrompt, buildPlanRevisionPrompt, buildProjectPlanPrompt, buildTaskPlanPrompt, detectPlanClarificationState, formatPlanForTranscript, mergeClarificationIntoPlanPrompt, readPlanDocument, writePlanDocument } from "../plan/plan-workflow.js";
...
import { PathAccessError, formatPathForDisplay, isPathOutsideProject, readScopeLabel, resolvePathFromContext, ensureLocalPathAllowed } from "../tools/workspace-safety.js";
import { formatAgentDetail, formatAgentList, formatHelpMessage, formatTaskDetail, formatModelList, formatModeMessage, formatSessionList, formatStyleMessage, formatTaskList, formatWatcherDetail, formatWatcherList, parseAppCommand } from "./commands.js";
...
export function App({ approvalManager, executionContextController, fleetManager, goalStore, initialSession, memoryStore, mcpManager, modelRuntime, projectRecord, projectRegistryStore, remoteManager, sessionRegistryStore, sessionStore, taskRunner, toolRegistry, timelineStore, watcherStore, workspaceRoot }) {
    const windowSize = useWindowSize();
    const layout = calculateTuiLayout(windowSize.columns, windowSize.rows);
    const [executionContext, setExecutionContext] = useState(() => executionContextController.get());
    ...
    const [appState, setAppState] = useState(() => toAppRuntimeState(modelRuntime.getSnapshot(), createRuntimeStateOptions(executionContextController.get(), projectRecord)));
    const [approvals, setApprovals] = useState([]);
    const [approvalStatus, setApprovalStatus] = useState(() => approvalManager.getStatus());
    ...
    const [fleetAgents, setFleetAgents] = useState(() => fleetManager?.listAgents() ?? []);
    ...
    const [planClarificationState, setPlanClarificationState] = useState(null);
    const [planReviewState, setPlanReviewState] = useState(null);
    ...
    const currentTaskIdRef = useRef(undefined);
    const currentSessionRef = useRef(initialSession);
    const executionContextRef = useRef(executionContextController.get());
    const appStateRef = useRef(toAppRuntimeState(modelRuntime.getSnapshot(), createRuntimeStateOptions(executionContextController.get(), projectRecord)));
    ...
    const planClarificationStateRef = useRef(null);
    const planReviewStateRef = useRef(null);
    const buildProgressRef = useRef(null);
    const clientIdRef = useRef(`tui-${process.pid}-${Date.now().toString(36)}`);
    const lastAgentPromptInputRef = useRef(undefined);
    const lastUserPromptRef = useRef("");
    const autopilotEnabledRef = useRef(false);
    const lastModeApprovalPresetRef = useRef("auto-safe");
    const sshToolOwnersRef = useRef(new Set());
    ...
    const agentModeLabel = isBusy || runningFleetCount > 0 || runningTaskCount > 0
        ? "AGENT MODE"
        : "Agent idle";
    const syncSshWorkerTools = (agents) => {
        if (!remoteManager) {
            return;
        }
        ...
        for (const agent of sshAgents) {
            ...
            toolRegistry.replaceOwnerTools(owner, createSshWorkerToolRegistrations(agent, owner, remoteManager));
        }
    ...
    useEffect(() => {
        const syncTools = () => {
            ...
            emitAppDebugLog(`loaded tools updated: ${nextTools.map((tool) => tool.name).join(", ")}`);
            startTransition(() => {
                setTools(nextTools);
            });
        };
    ...
    useEffect(() => {
        return modelRuntime.subscribe((snapshot) => {
            ...
            commitAppState(toAppRuntimeState(snapshot, createRuntimeStateOptions(executionContextRef.current, projectRecord)));
        });
    }, [modelRuntime, projectRecord]);
    useEffect(() => {
        commitAppState(toAppRuntimeState(modelRuntime.getSnapshot(), createRuntimeStateOptions(executionContext, projectRecord)));
    ...
    const commitApprovalStatus = (nextStatus) => {
        setApprovalStatus({
            ...nextStatus,
    ...
    const commitAutopilotEnabled = (enabled) => {
        autopilotEnabledRef.current = enabled;
        setAutopilotEnabled(enabled);
    };
    const commitExecutionContext = (nextContext) => {
        executionContextRef.current = nextContext;
        setExecutionContext(nextContext);
        commitAppState(toAppRuntimeState(modelRuntime.getSnapshot(), createRuntimeStateOptions(nextContext, projectRecord)));
    };
    const syncApprovalPolicy = (nextPolicy) => {
        approvalManager.setPolicy(nextPolicy);
        commitExecutionContext(executionContextController.update({
    ...
    const commitExecutionMode = (nextMode) => {
        ...
        emitAppDebugLog(`execution mode ${nextMode} tools: ${filterToolsForExecutionMode(tools, nextMode)
            .map((tool) => tool.name)
            .join(", ")}`);
        commitApprovalStatus(approvalManager.getStatus());
        setExecutionMode(nextMode);
    };
    const commitTaskContext = (nextTaskId) => {
        currentTaskIdRef.current = nextTaskId;
        approvalManager.setActiveTask(nextTaskId);
        commitApprovalStatus(approvalManager.getStatus());
    };
    const resolveModeApprovalPreset = (nextMode, options) => {
        if (nextMode === "plan") {
            return "strict";
        }
    ...
    const applyDefaultApprovalPolicyForMode = (nextMode, options) => {
    ...
    const setAutopilotState = (enabled, policy) => {
    ...
    const commitBuildProgress = (nextBuildProgress) => {
    ...
    const restoreSessionWorkflowState = (entry) => {
        ...
        if (savedState.workflowState &&
            ...
            typeof savedState.workflowState.currentStep === "number") {
            commitWorkflowState(savedState.workflowState);
        }
        if (savedState.planClarificationState && typeof savedState.planClarificationState === "object") {
            commitPlanClarificationState(savedState.planClarificationState);
        }
        if (savedState.planReviewState &&
            typeof savedState.planReviewState === "object") {
            commitPlanReviewState(savedState.planReviewState);
        }
    ...
    const refreshRemoteAgents = async () => {
        if (!remoteManager) {
            return [];
        }
    ...
    const approvePendingForPreset = async (preset) => {
    ...
    const grantApprovalScope = async (scope) => {
        ...
        if (!granted) {
            throw new Error(effectiveScope === "task"
    ...
    const resumeBlockedTask = async (transcriptPrompt = "Continue with the approved task.") => {
    ...
    const tryHandleNaturalApprovalInput = async (input) => {
        const pendingApprovals = await approvalManager.listPending();
    ...
    const tryHandleNaturalSshInput = async (input) => {
        if (!remoteManager) {
            return false;
        }
        ...
        await executeAgentPrompt({
            prompt: buildAgenticSshPrompt(intent.agent, intent.command),
            rememberUserPrompt: true,
            systemMessages: [
                {
                    role: "system",
                    content: [
                        `The user explicitly requested a command on SSH worker "${intent.agent.id}".`,
                        `Call ${createSshRunToolName(intent.agent.id)} with command ${JSON.stringify(intent.command)}.`,
                        "Read the tool output before answering.",
                        "Do not use local run_command for this request."
                    ].join("\n")
                }
            ],
            toolRegistryOverride: createSingleToolRegistry(toolRegistry, createSshRunToolName(intent.agent.id)),
            transcriptPrompt: input
        });
        return true;
    };
    ...
    const cancelWatchers = async (watcherIds, detail) => {
    ...
    const stopAllTasks = async (action) => {
        let changedCount = 0;
        for (const task of taskRunner.listTasks()) {
            ...
            if (nextTask.watcherIds.length > 0) {
                await cancelWatchers(nextTask.watcherIds, `Task ${nextTask.id} was ${action === "cancel" ? "cancelled" : "stopped"} by the user.`);
            }
        }
        return changedCount;
    };
    const changeDirectory = async (nextPath) => {
    ...
    const grantScopeAccess = async (grant) => {
        const grantRecord = {
            createdAt: Date.now(),
            hostId: grant.hostId,
    ...
    const revokeScopeAccess = async (grantIdOrScope) => {
        const current = executionContextController.revokeGrant(grantIdOrScope);
        if (current.scope === "remote-host" &&
        ...
        if ((grantIdOrScope.toLowerCase() === current.scope ||
    ...
    const assignFleetTask = async (role, task) => {
        if (!fleetManager) {
    ...
    const buildInteractiveTaskRecord = (input, taskId, transcriptPrompt) => {
    ...
    const readInteractiveTaskControlDecision = (taskId) => {
    ...
    const executeTrackedTask = async (task, options) => {
        const nextTask = options?.retry
        ...
        try {
            switch (command.type) {
                ...
                case "cwd":
                    if (!disposedRef.current) {
                        startTransition(() => {
                            appendSystemMessage(formatExecutionContextStatus(executionContextRef.current, projectRecord, executionContextController.getGrants()));
                        });
                    }
                    return true;
                case "cd": {
                    ...
                    if (!disposedRef.current) {
                        startTransition(() => {
                            appendSystemMessage(`Current directory set to ${formatPathForDisplay(nextContext, nextContext.cwd)}.`);
                        });
                    }
                    return true;
                }
                case "scope":
                    {
                        ...
                        if (!disposedRef.current) {
                            startTransition(() => {
                                appendSystemMessage(formatScopeStatus({
                ...
                case "scope_grant": {
                    ...
                    if (!disposedRef.current) {
                        startTransition(() => {
                            appendSystemMessage(`Scope set to ${readScopeLabel(nextContext.scope)}${nextContext.activeRemoteHostId ? ` (${nextContext.activeRemoteHostId})` : ""}.`);
                        });
                    }
                    return true;
                }
                case "scope_revoke": {
                    ...
                    if (!disposedRef.current) {
                        startTransition(() => {
                            appendSystemMessage(`Revoked "${command.grantIdOrScope}". Active scope is ${readScopeLabel(nextContext.scope)}.`);
                        });
                    }
                    return true;
                }
                case "approvals": {
                    ...
                    if (!disposedRef.current) {
                        startTransition(() => {
                            appendSystemMessage(formatApprovalList(nextApprovals, approvalManager.getStatus()));
                        });
                    }
                    return true;
                }
                case "approve_scope": {
                    const hadPendingApprovals = (await approvalManager.listPending()).length > 0;
                    await grantApprovalScope(command.scope);
                    if (!disposedRef.current) {
                        startTransition(() => {
                            appendSystemMessage(command.scope === "once"
                ...
                case "approve_policy": {
                    ...
                    if (!disposedRef.current) {
                        startTransition(() => {
                            appendSystemMessage(`Approval policy set to ${nextPolicy.preset ?? command.policy}.`);
                        });
                    }
                    if (hadPendingApprovals &&
                        (await approvalManager.listPending()).length === 0) {
                ...
                case "autopilot_on": {
                    ...
                    if (!disposedRef.current) {
                        startTransition(() => {
                            appendSystemMessage(`Autopilot enabled with ${nextPolicy} approval policy for this session.`);
                        });
                    }
                    if (hadPendingApprovals &&
                        (await approvalManager.listPending()).length === 0) {
                ...
                case "autopilot_off": {
                    ...
                    if (!disposedRef.current) {
                        startTransition(() => {
                            appendSystemMessage(`Autopilot disabled. Active approval policy is ${nextPolicy}.`);
                        });
                    }
                    return true;
                }
                case "autopilot_status": {
                    ...
                    if (!disposedRef.current) {
                        startTransition(() => {
                            appendSystemMessage(formatAutopilotStatus({
                                approvalStatus: approvalManager.getStatus(),
                                autopilotEnabled: autopilotEnabledRef.current,
                                context: executionContextRef.current,
                                executionMode: executionModeRef.current,
                                isBusy,
                                pendingApprovalCount: nextApprovals.filter((approval) => approval.state === "pending").length
                            }));
                        });
                    }
                    return true;
                ...
                case "watcher_cancel": {
                    if (isAllTarget(command.id)) {
                        ...
                        const cancelledCount = await cancelWatchers(nextWatchers.map((watcher) => watcher.id), "Cancelled from /watcher cancel all.");
                        if (!disposedRef.current) {
                            startTransition(() => {
                ...
                case "goal_adopt": {
                    ...
                    await goalStore.saveGoal({
                        ...goal,
                        nextBestAction: goal.nextBestAction ?? "Run /plan and describe the work for this goal.",
                        state: "planning"
                    });
                    const nextGoals = await refreshGoals();
                    if (!disposedRef.current) {
                        startTransition(() => {
                            appendSystemMessage(`Goal "${goal.title}" adopted. Next action: ${goal.nextBestAction ?? "Run /plan for this goal."}`);
                        });
                    }
                    void nextGoals;
                    return true;
                ...
                case "fleet_assign": {
                    ...
                    if (!disposedRef.current) {
                        startTransition(() => {
                            appendSystemMessage(`Assigned sub-agent ${agent.id} (${agent.role}) to: ${agent.task}`);
                        });
                    }
                    return true;
                }
                ...
                case "orchestrator_health": {
                    ...
                    if (!disposedRef.current) {
                        startTransition(() => {
                            appendSystemMessage(formatOrchestratorStatus({
                                approvals: nextApprovals,
                                context: executionContextRef.current,
                                executionMode,
                                fleetAgents,
                ...
                case "remote_status": {
                    ...
                    if (!disposedRef.current) {
                        startTransition(() => {
                            appendSystemMessage(agent ? formatRemoteDetail(agent) : `Remote worker "${command.id}" was not found.`);
                        });
                    }
                    return true;
                }
                ...
                case "mcp_refresh": {
                    ...
                    if (!disposedRef.current) {
                        startTransition(() => {
                            appendSystemMessage(formatMcpReloadSummary("Reloaded MCP registry.", result));
                        });
                    }
                    await timelineStore?.record({
                        sessionId: currentSessionRef.current.id,
                ...
                case "plan":
                    ...
                    if (!disposedRef.current) {
                        startTransition(() => {
                            ...
                            appendSystemMessage("PLAN mode enabled. Describe the task you want planned. I will inspect the codebase in read-only mode, ask clarifying questions if needed, then generate TODO + plan.md.", true);
                        });
                    }
                    return true;
                case "build": {
                ...
                case "resume": {
                    ...
                    const plan = await readPlanDocument(workspaceRoot);
                    const buildState = deriveResumableBuildStateFromProgress(buildProgressRef.current, plan) ??
                ...
                case "memory_global_show": {
                    ...
                    const memories = (await extendedMemoryStore.listMemories()).filter((memory) => memory.scope === "global");
                    if (!disposedRef.current) {
                        startTransition(() => {
                ...
                case "memory_global_remember": {
                    ...
                    if (!disposedRef.current) {
                        startTransition(() => {
                            appendSystemMessage(`Saved global memory ${memory.id}: ${memory.text}`);
                        });
                    }
                    return true;
                }
                ...
                case "memory_search": {
                    ...
                    if (!disposedRef.current) {
                        startTransition(() => {
                            appendSystemMessage(formatMemoryEntries(memories, `Memory search: ${command.keyword}`));
                        });
                    }
                    return true;
                }
                ...
                case "timeline_filter": {
                    ...
                    const events = await timelineStore.filterByType(command.filterType);
                    if (!disposedRef.current) {
                        startTransition(() => {
                            appendSystemMessage(formatTimelineEntries(events, `Timeline: ${command.filterType}`));
                        });
                    }
                    return true;
                }
                    ...
                    if (!disposedRef.current) {
                        startTransition(() => {
                            ...
                            if (sessionRegistryEntry) {
                                const sessionMetadata = isApprovalManagerMetadata(sessionRegistryEntry.metadata)
                                    ? sessionRegistryEntry.metadata
                                    : undefined;
                                approvalManager.restore({
                                    activeSessionId: session.id,
                                    activeTaskId: typeof sessionRegistryEntry.resumablePlanState?.activeTaskId === "string"
                                        ? sessionRegistryEntry.resumablePlanState.activeTaskId
                                        : undefined,
                                    mode: sessionRegistryEntry.mode,
                                    policy: sessionMetadata
                                ...
                                lastModeApprovalPresetRef.current = resolveModeApprovalPreset(sessionRegistryEntry.mode, {
                                    autopilotEnabled: false
                                });
                                commitExecutionContext(executionContextController.update({
                                    activeRemoteHostId: sessionRegistryEntry.activeRemoteHostId,
                                    allowedRoots: sessionRegistryEntry.allowedRoots,
                                    approvalPolicy: approvalManager.getStatus().policy,
            ...
            const message = error instanceof PathAccessError
    ...
    const executeAgentPrompt = async (input) => {
        ...
        const taskId = createTaskId(currentSessionRef.current.id, input.prompt, transcriptPrompt);
        const baseTranscript = [...transcriptRef.current];
        const userEntry = transcriptPrompt.trim().length > 0
        ...
        let trackedTask = await taskRunner.saveTask(buildInteractiveTaskRecord(input, taskId, transcriptPrompt));
        ...
        try {
            const result = await runAgentTurnWithRateLimitRecovery({
                ...
                onRetry: async ({ attempt, delayMs, error }) => {
                    ...
                    turnTranscript = userEntry
                        ? appendTranscriptEntry(baseTranscript, userEntry)
                        : [...baseTranscript];
                    turnTranscript = appendTranscriptEntry(turnTranscript, createSystemEntry(`Provider rate limited this turn. Retrying in ${formatRetryDelay(delayMs)} (attempt ${attempt}/${RATE_LIMIT_RETRY_DELAYS_MS.length}).`, true));
                    emitAppDebugLog(`retrying after rate limit in ${delayMs}ms: ${error instanceof Error ? error.message : String(error)}`);
                    await timelineStore?.record({
            ...
            const relevantPendingApprovals = blockedApprovalIds.size > 0
                ? (await approvalManager.listPending()).filter((request) => {
                    return (blockedApprovalIds.has(request.id) ||
            ...
            trackedTask = await taskRunner.saveTask({
                ...trackedTask,
                approvalRequestIds: [...blockedApprovalIds],
                blockedReason: error instanceof AgentControlError ? message : undefined,
                currentStep: undefined,
                lastError: error instanceof AgentControlError ? undefined : message,
                lastEventAt: Date.now(),
                lastRunAt: Date.now(),
                requestedAction: undefined,
                runCount: trackedTask.runCount + 1,
    ...
    const startPlanWorkflow = async (taskPrompt) => {
        ...
        if (clarificationState) {
            if (!disposedRef.current) {
                startTransition(() => {
                    ...
                    appendSystemMessage(buildPlanClarificationMessage(clarificationState), true);
                });
            }
            return;
        }
    ...
    const reviseExistingPlan = async (reviewState, feedback) => {
        ...
        startTransition(() => {
            ...
            appendSystemMessage(`Revising the current plan using your feedback: ${feedback}`, true);
        });
        const result = await executeAgentPrompt({
            prompt: buildPlanRevisionPrompt(reviewState.plan, feedback, reviewState.originalPrompt),
            rememberUserPrompt: false,
            systemMessages: [
                {
                    role: "system",
                    content: "Return only the updated markdown plan. Do not include commentary before or after the plan."
                }
            ],
        ...
        try {
            ...
            if (!disposedRef.current) {
                startTransition(() => {
                    ...
                    appendSystemMessage(`${formatPlanForTranscript(updatedPlan)}\n\nReply with “implement it” or /build to execute, or send more plan edits to revise it again.`, true);
                });
            }
            await persistSessionSnapshot();
        }
            ...
            if (!disposedRef.current) {
                startTransition(() => {
                    appendSystemMessage(error instanceof Error
                        ...
                        : "Failed to save revised plan.md.", true);
                });
            }
        }
    };
    const generateProjectPlan = async (taskPrompt) => {
        const planningToolRegistry = createExecutionModeToolRegistry(toolRegistry, "plan");
        startTransition(() => {
            commitExecutionMode("plan");
            commitPendingPlanPrompt(false);
            ...
            appendSystemMessage(taskPrompt
                ? `PLAN mode enabled. Analyzing: ${taskPrompt}`
                : "PLAN mode enabled. Inspecting the project in read-only mode and generating plan.md.", true);
        });
        const result = await executeAgentPrompt({
    ...
    const executeSavedPlan = async (plan, options) => {
        ...
        startTransition(() => {
            ...
            commitBuildProgress({
                ...
                lastCompletedStepIndex: completedStepIndexes.length > 0 ? completedStepIndexes.at(-1) : undefined,
                status: "running",
                updatedAt: Date.now()
            });
            commitWorkflowState({
    ...
    const submitMessage = (rawValue) => {
        void (async () => {
            ...
            if (planClarificationStateRef.current &&
                input.length > 0) {
                const combinedPrompt = mergeClarificationIntoPlanPrompt(planClarificationStateRef.current, input);
            ...
            if (detectFleetDelegationIntent(input) && fleetManager) {
                ...
                if (!disposedRef.current) {
                    startTransition(() => {
                        appendSystemMessage(`Delegated to sub-agent ${agent.id} (${agent.role}). Use /fleet status to track progress.`, true);
                    });
                }
                return;
            }
    ...
    return (_jsx(AppRuntimeStateProvider, { value: appState, children: _jsxs(Box, { flexDirection: "column", height: windowSize.rows, paddingX: 1, children: [_jsx(Header, { activeToolCount: activeToolCount + runningFleetCount, agentModeLabel: agentModeLabel, approvalLabel: createApprovalHeaderLabel(approvalStatus, pendingApprovalCount), autoModeLabel: autoModeLabel, autopilotLabel: createAutopilotHeaderLabel(autopilotEnabled, isBusy), cwdLabel: createCwdHeaderLabel(executionContext), executionModeLabel: executionModeLabel, projectLabel: createProjectHeaderLabel(projectRecord, executionContext), scopeLabel: createScopeHeaderLabel(executionContext), statusLabel: createStatusLabel(phase, step), statusTone: phase === "ready" ? "ready" : "busy", taskCount: taskCount + fleetAgents.length, toolCount: deferredTools.length, workflowProgressLabel: workflowState.progressLabel, width: layout.frameWidth }), _jsxs(Box, { flexDirection: layout.isWide ? "row" : "column", height: layout.bodyHeight, children: [activeFlow ? (_jsx(FlowPanel, { flow: activeFlow.flow, height: layout.mainHeight, width: layout.mainWidth, onCancel: () => {
                                commitActiveFlow(null);
                                setComposerValue("");
                            }, onMoveSelection: moveActiveFlowSelection, onSubmitSelection: submitActiveFlowSelection })) : (_jsx(OutputPanel, { entries: transcript, height: layout.mainHeight, isStreaming: isStreaming, width: layout.mainWidth })), _jsx(Box, { marginLeft: layout.isWide ? 1 : 0, marginTop: layout.isWide ? 0 : 1, children: _jsx(Sidebar, { activeTools: toolActivity, agentModeLabel: agentModeLabel, height: layout.sideHeight, lastToolSummary: lastToolSummary, currentWorkflowLabel: workflowState.currentStepLabel, executionModeLabel: executionModeLabel, autoModeLabel: autoModeLabel, autoModeLastAction: autoModeLastAction, autoModeNextRunLabel: autoModeNextRunLabel, phaseLabel: createPhaseLabel(phase), responseModeLabel: formatResponseModeLabel(mode), sessionLabel: createSessionLabel(currentSession), step: step, taskLabel: `${taskCount} tasks • ${runningTaskCount} running • ${fleetAgents.length} agents`, tools: deferredTools, workflowProgressLabel: workflowState.progressLabel, width: layout.sideWidth }) })] }), _jsx(InputBar, { footer: composerInputState.footer, focusInput: activeFlowStep?.inputType !== "select" && activeFlowStep?.inputType !== "confirm", isBusy: isBusy, isStreaming: isStreaming, mask: composerInputState.mask, placeholder: composerInputState.placeholder, subtitle: composerInputState.subtitle, title: composerInputState.title, value: composerValue, width: layout.frameWidth, onChange: setComposerValue, onSubmit: submitMessage })] }) }));
}
...
export async function runAgentTurnWithRateLimitRecovery(input) {
    return runWithRateLimitRecovery({
        onRetry: async ({ attempt, delayMs, error }) => {
            emitAppDebugLog(`rate limit detected; retry ${attempt}/${RATE_LIMIT_RETRY_DELAYS_MS.length} in ${delayMs}ms`);
            await input.onRetry?.({
                attempt,
                delayMs,
                error
...
function getExtendedMemoryStore(memoryStore) {
    const candidate = memoryStore;
    if (typeof candidate.rememberGlobal === "function" &&
...
function formatMemoryEntries(memories, title) {
...
function formatCompactedMemory(summary) {
    ...
    lines.push("");
    lines.push(`Current task: ${Object.keys(summary.currentTask).length > 0
...
function formatTimelineEntries(events, title) {
    if (events.length === 0) {
        return `${title}\n\nNo timeline events recorded yet.`;
    }
    const lines = [title];
    for (const event of events) {
        lines.push(`${new Date(event.createdAt).toLocaleTimeString([], {
            hour: "2-digit",
            minute: "2-digit"
...
function formatGoalList(goals) {
    ...
    for (const goal of goals) {
        lines.push(`${goal.id}  [${goal.state}]  ${goal.title}  ${goal.estimatedEffort}`);
    }
    lines.push("");
...
function formatFleetStatus(agents, enabled) {
    const lines = [
    ...
    for (const agent of agents) {
        lines.push(`${agent.id}  [${agent.state}]  ${agent.role}  ${agent.task}`);
        if (agent.summary) {
...
function formatRemoteList(agents) {
    if (agents.length === 0) {
        return "Remote workers\n\nNo remote workers configured yet.";
    }
    const lines = ["Remote workers"];
    for (const agent of agents) {
        lines.push(`${agent.id.padEnd(14, " ")} ${agent.role.padEnd(8, " ")} ${agent.status.padEnd(9, " ")} ${agent.host}`);
        lines.push(`  tool=${createSshRunToolName(agent.id)}`);
    }
    return lines.join("\n");
}
function formatRemoteDetail(agent) {
    return [
...
function formatSshAddedMessage(name, host) {
    return [
...
function formatSshRunResult(name, command, result) {
    const lines = [
        `SSH ${name}: ${command}`,
...
function buildAgenticSshPrompt(agent, command) {
    return [
        `Run this command on SSH worker "${agent.id}" using ${createSshRunToolName(agent.id)}:`,
        command,
        "",
        "After the tool returns, answer using only the observed output."
    ].join("\n");
}
function buildRemoteWorkerContextMessage(agents) {
    const sshAgents = agents.filter((agent) => agent.host.trim().length > 0);
    ...
    return {
        ...
        content: [
            ...
            ...sshAgents.map((agent) => `- ${agent.id}: host=${agent.host}, status=${agent.status}, tool=${createSshRunToolName(agent.id)}`),
...
function createSingleToolRegistry(toolRegistry, toolName) {
    return {
        listTools() {
            return toolRegistry
                .listTools()
        ...
        executeTool(name, input) {
            if (name !== toolName) {
                return Promise.resolve({
                    content: `Use ${toolName} for this SSH worker request; "${name}" is not available for this turn.`,
                    isError: true
                });
            }
...
function createSshWorkerToolRegistrations(agent, owner, remoteManager) {
...
function createSshToolOwner(agentId) {
    return `ssh:${agentId}`;
}
function createSshRunToolName(agentId) {
...
function readToolString(input, key) {
    const value = input[key];
    return typeof value === "string" && value.trim().length > 0
        ? value.trim()
        : undefined;
}
function parseNaturalSshCommand(input, agents) {
    ...
    for (const agent of sshAgents) {
        ...
        for (const name of names) {
            ...
            const commandOnHost = trimmed.match(new RegExp(`^(?:please\\s+)?(?:do|run|execute)\\s+(?:command\\s+|comand\\s+|cmd\\s+)?(.+?)\\s+(?:on|in|at)\\s+(?:ssh\\s+)?${escapedName}\\s*$`, "i"));
            const hostThenCommand = trimmed.match(new RegExp(`^(?:please\\s+)?(?:on|in|at)\\s+(?:ssh\\s+)?${escapedName}\\s+(?:do|run|execute)\\s+(?:command\\s+|comand\\s+|cmd\\s+)?(.+?)\\s*$`, "i"));
            const command = cleanNaturalShellCommand(commandOnHost?.[1] ?? hostThenCommand?.[1]);
            if (command) {
                return {
                    agent,
                    command
...
function cleanNaturalShellCommand(value) {
    const command = value
        ?.trim()
        .replace(/^`+|`+$/g, "")
        .trim();
...
function formatOrchestratorStatus(input) {
    ...
    const pendingApprovals = input.approvals.filter((approval) => approval.state === "pending").length;
    const activeWatchers = input.watchers.filter((watcher) => watcher.status === "active" ||
...
function formatExecutionContextStatus(context, project, grants) {
    const lines = [
    ...
    if (grants.length > 0) {
        ...
        for (const grant of grants) {
            lines.push(`${grant.id}  ${readScopeLabel(grant.scope)}${grant.hostId ? ` (${grant.hostId})` : ""}${grant.root ? ` -> ${grant.root}` : ""}`);
        }
    }
    return lines.join("\n");
}
...
function formatApprovalList(approvals, status) {
    const lines = [
        "Approvals",
        "",
    ...
    for (const approval of approvals) {
        lines.push("");
        lines.push(`${approval.id}  [${approval.state}]  ${approval.kind}  ${approval.summary}`);
        lines.push(`scope=${approval.scope ?? "unknown"}  safety=${approval.safetyLevel ?? "unknown"}${approval.resource ? `  resource=${approval.resource}` : ""}`);
        lines.push(approval.detail);
    }
    return lines.join("\n");
}
function formatPathAccessError(error, context) {
    const diagnostics = [
...
function createRuntimeStateOptions(context, project) {
    return {
        cwd: context.cwd,
...
function buildPlanClarificationMessage(clarificationState) {
    const lines = [
...
function buildPlanUnderstandingMessage(taskPrompt) {
...
function isPlanImplementationInput(input) {
    const normalized = input.trim().toLowerCase();
    return (normalized === "implement it" ||
...
export function deriveResumableBuildState(transcript, plan) {
    ...
    if (relevantEntries.some((entry) => entry.content.includes("BUILD mode completed every step in plan.md."))) {
        return {
            canResume: false,
    ...
    const completedSteps = plan.steps.filter((step) => completedStepIndexes.has(step.index));
    const startStepIndex = failedStepIndex ??
...
function deriveResumableBuildStateFromProgress(progress, plan) {
    if (!progress || progress.status === "idle") {
        return undefined;
    }
    const completedStepIndexes = new Set(progress.completedStepIndexes.filter((index) => plan.steps.some((step) => step.index === index)));
    const completedSteps = plan.steps.filter((step) => completedStepIndexes.has(step.index));
    ...
    const startStepIndex = progress.lastAttemptedStepIndex && !completedStepIndexes.has(progress.lastAttemptedStepIndex)
...
function getEntriesSinceLastBuildStart(transcript) {
    const buildStartIndex = [...transcript]
        .map((entry, index) => ({ entry, index }))
        .reverse()
        .find(({ entry }) => entry.content.includes("BUILD mode enabled. Executing"))?.index;
...
function createAutopilotHeaderLabel(enabled, isBusy) {
    if (!enabled) {
        return "Autopilot off";
    }
    return isBusy ? "AUTOPILOT ACTIVE" : "AUTOPILOT ON";
}
function createCwdHeaderLabel(context) {
...
function formatRemoteDirectoryError(targetPath, stderr) {
...
function createSshWorkerFlowSteps() {
    return [
        {
            key: "name",
            title: "Worker Name",
        ...
        },
        {
            key: "sshPassword",
            title: "Password",
            description: "Enter the SSH password. It is stored in the local worker registry for non-interactive commands.",
            inputType: "text",
            mask: "*",
        ...
        },
        {
            ...
            description: (state) => `Save SSH worker ${String(state.name ?? "this worker")} and make it available to remote tools.`,
            inputType: "confirm",
            options: [
                {
                    label: "Save worker",
...
function createDefaultSshWorkerDraft() {
    return {
        host: "",
        name: ""
    };
...
function buildSshWorkerFromFlow(flow) {
    return {
...
function readOptionalPort(flow, key) {
...
function isPersistedBuildProgress(value) {
    if (typeof value !== "object" || value === null) {
        return false;
    }
    const candidate = value;
    return (Array.isArray(candidate.completedStepIndexes) &&
...
function isApprovalManagerMetadata(value) {
    if (typeof value !== "object" || value === null) {
        return false;
    }
    const metadata = value;
    const approvalStatus = metadata.approvalStatus;
    return (typeof approvalStatus === "object" &&
        approvalStatus !== null &&
...
function createTaskId(sessionId, prompt, transcriptPrompt) {
...
function formatPendingApprovalMessage(approvals) {
...
function formatApprovalPolicyPreset(preset) {
    switch (preset) {
        case "ask-every-time":
...
function formatApprovalGrantStatus(status) {
...
export function parseApprovalShortcutIntent(input, context) {
