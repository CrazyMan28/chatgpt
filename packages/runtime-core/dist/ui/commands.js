...
const COMMAND_REFERENCE = [
    ...
    {
        command: "/tasks",
        usage: "/tasks | /task show <id> | /task pause <id> | /task resume <id> | /task stop <id|all> | /task cancel <id|all> | /task retry <id> | /task start <id|goal>",
        description: "List and control tracked tasks."
    },
    {
        command: "/watchers",
        usage: "/watchers | /watcher show <id> | /watcher cancel <id|all>",
...
export function formatTaskList(tasks, currentSessionId) {
    ...
    for (const task of tasks) {
        ...
        const taskLabel = task.kind === "auto"
            ? "[auto]"
            : task.kind === "interactive"
                ? "[interactive]"
                : "[task]";
        const interval = task.kind === "interactive" ? "manual" : `every ${formatInterval(task.intervalMinutes)}`;
        ...
        const summary = task.lastError ??
            ...
            task.title;
        lines.push(`${marker} ${taskLabel} ${task.id}  ${task.state}${task.isRunning ? " (active)" : ""}  ${interval}  next ${nextRun}  last ${lastRun}`);
        lines.push(`  ${task.title}  step=${task.currentStep ?? "-"}  agent=${task.assignedAgentId ?? "-"}  approvals=${task.approvalRequestIds.length}  watchers=${task.watcherIds.length}  retries=${task.retries}`);
        if (summary.trim().length > 0) {
            lines.push(`  ${summary}`);
        }
    }
    lines.push("");
    lines.push("Use /task show <id> for detail, /task pause|resume|stop|cancel|retry <id> for control, /task stop all or /task cancel all for bulk stop, /task start <goal> for a manual run, /schedule <minutes> <prompt> for recurring tasks, or /auto <minutes> for autonomous mode.");
    return lines.join("\n");
}
...
export function formatWatcherList(watchers) {
    if (watchers.length === 0) {
        return "No watchers are running.";
    }
    const lines = ["Watchers"];
    for (const watcher of watchers) {
        lines.push(`${watcher.id}  ${watcher.status}  ${watcher.type}  ${watcher.summary}`);
        lines.push(`  target=${watcher.target ?? "-"}  scope=${watcher.scope ?? "-"}  task=${watcher.taskId ?? "-"}  retries=${watcher.retryCount ?? 0}`);
    }
    return lines.join("\n");
}
export function formatWatcherDetail(watcher) {
    return [
        `Watcher ${watcher.id}`,
...
export function formatAgentList(agents, enabled) {
    if (agents.length === 0) {
        return `Fleet ${enabled ? "enabled" : "disabled"}.\n\nNo agents yet.`;
    }
    const lines = [`Fleet ${enabled ? "enabled" : "disabled"}`, ""];
    for (const agent of agents) {
        lines.push(`${agent.id}  ${agent.state}  ${agent.role}  task=${agent.task}`);
        lines.push(`  cwd=${agent.currentCwd ?? "-"}  scope=${agent.currentScope ?? "-"}  blockers=${agent.blockers?.length ?? 0}  changed=${agent.changedFiles.length}`);
        if (agent.summary) {
...
export function formatStyleMessage(style) {
...
function parsePlanCommand(input) {
...
function parseFleetCommand(input) {
    const [, subcommand = "", ...rest] = input.split(/\s+/);
    ...
    switch (normalizedSubcommand) {
        ...
        case "assign": {
            ...
            const role = normalizedRole === "frontend" ||
                normalizedRole === "backend" ||
                normalizedRole === "tester" ||
                normalizedRole === "docs"
                ? normalizedRole
                : "general";
            const task = role === "general" ? payload : taskParts.join(" ").trim();
            if (task.length === 0) {
                return {
                    type: "invalid",
                    message: "Usage: /fleet assign <role> <task>"
    ...
    switch (normalizedSubcommand) {
        ...
        case "stop":
            return value.length > 0
                ...
                : { type: "invalid", message: "Usage: /task stop <id|all>" };
        case "cancel":
            return value.length > 0
                ? { id: value, type: "task_cancel" }
                : { type: "invalid", message: "Usage: /task cancel <id|all>" };
        case "retry":
            return value.length > 0
        ...
        default:
            return {
                type: "invalid",
                message: "Usage: /task show <id> | /task pause <id> | /task resume <id> | /task stop <id|all> | /task cancel <id|all> | /task retry <id> | /task start <id|goal>"
            };
    }
}
function parseWatcherCommand(input) {
    const [, subcommand = "", ...rest] = input.split(/\s+/);
    const normalizedSubcommand = subcommand.toLowerCase();
    const value = rest.join(" ").trim();
    switch (normalizedSubcommand) {
        ...
        case "cancel":
            return value.length > 0
                ...
                : { type: "invalid", message: "Usage: /watcher cancel <id|all>" };
        default:
            return {
                type: "invalid",
                message: "Usage: /watchers | /watcher show <id> | /watcher cancel <id|all>"
            };
    }
}
function parseAgentCommand(input) {
    ...
    switch (normalizedSubcommand) {
        ...
        default:
            return {
                type: "invalid",
                message: "Usage: /agent show <id> | /agent pause <id> | /agent resume <id> | /agent stop <id> | /agent restart <id>"
            };
    }
}
function parseScopeCommand(input) {
    const [, subcommand = "", ...rest] = input.split(/\s+/);
    const normalizedSubcommand = subcommand.toLowerCase();
    ...
    return {
        type: "invalid",
        message: "Usage: /scope | /scope grant <workspace|home|full-machine|remote-host> [host] | /scope revoke <scope|id>"
    };
}
function parseOrchestratorCommand(input) {
...
function parseRemoteCommand(input) {
    const [, subcommand = "", ...rest] = input.split(/\s+/);
    const normalizedSubcommand = subcommand.toLowerCase();
    switch (normalizedSubcommand) {
        case "list":
        ...
        default:
            return {
                type: "invalid",
                message: "Usage: /remote add <id> <host> [username] [port] | list | status <id> | connect <id> | disconnect <id> | remove <id>"
            };
    }
}
function parseSshCommand(input) {
    ...
    switch (normalizedSubcommand) {
        ...
        default:
            return {
                type: "invalid",
                message: "Usage: /ssh add [name host [username] [port]] | list | test <name> | run <name> <command> | remove <name>"
            };
    }
}
function parseMcpCommand(input) {
    ...
    switch (normalizedSubcommand) {
        ...
        default:
            return {
                type: "invalid",
                message: "Usage: /mcp add | /mcp list | /mcp refresh | /mcp marketplace | /mcp info <name> | /mcp install <name> | /mcp enable <name> | /mcp disable <name> | /mcp remove <name> | /mcp edit <name>"
            };
    }
}
...
function parseMemoryCommand(input) {
    ...
    return {
        type: "invalid",
        message: "Usage: /memory show | /memory clear | /memory compact | /memory global show | /memory global remember <text> | /memory global forget <key> | /memory search <keyword>"
    };
}
function parseTimelineCommand(input) {
...
function parseApprovalAction(input, action) {
    const value = input.replace(new RegExp(`^\\/${action}\\s*`, "i"), "").trim();
    const normalizedValue = value.toLowerCase();
    if (action === "approve") {
        if (normalizedValue === "once" ||
        ...
        if (normalizedValue === "approve-all" ||
            ...
            return {
                policy: normalizedValue === "approve-all"
                    ? "approve-all"
                    : normalizedValue === "approve-task"
                        ? "approve-task"
                        : normalizedValue === "approve-session"
...
function parseAutopilotCommand(input) {
    ...
    if (normalizedSubcommand === "on") {
        if (normalizedPolicy.length === 0 ||
            ...
            return {
                policy: normalizedPolicy.length > 0
                    ? normalizedPolicy
                    : undefined,
                type: "autopilot_on"
            };
        ...
        return {
            type: "invalid",
            message: "Usage: /autopilot on [full|auto-safe|auto-safe-medium|strict] | /autopilot off | /autopilot status"
        };
    }
    return {
        type: "invalid",
