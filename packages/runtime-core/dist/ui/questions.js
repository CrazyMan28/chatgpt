...
export const ASK_QUESTION_TOOL_DEFINITION = {
    ...
    description: "Pause and ask the user a real interactive question when progress depends on a meaningful decision. Use this instead of guessing for high-impact choices.",
    inputSchema: {
        type: "object",
...
export function createQuestion(input) {
    ...
    const recommendedOption = normalizeRecommendedOption(input.recommendedOption, options);
    return {
...
export function createQuestionQueue(input) {
    const pendingQuestion = input.questions.find((question) => question.state === "pending");
    return {
...
export function getPendingQuestions(queue) {
    const questions = Array.isArray(queue?.questions) ? queue.questions : [];
    return questions.filter((question) => question.state === "pending");
}
export function getActiveQuestion(queue) {
    if (!queue) {
        return undefined;
    }
    const questions = Array.isArray(queue.questions) ? queue.questions : [];
    const activeQuestion = questions.find((question) => question.id === queue.activeQuestionId && question.state === "pending");
    return activeQuestion ?? getPendingQuestions(queue)[0];
}
export function getQuestionProgress(queue, question) {
    if (!queue || !question) {
        return undefined;
    }
    const questions = Array.isArray(queue.questions) ? queue.questions : [];
...
export function getQuestionRows(question) {
...
export function getRecommendedQuestionRowIndex(question) {
    ...
    const index = options.findIndex((option) => option.id === recommended.optionId ||
...
export function resolveQuestionTextAnswer(question, input) {
...
export function resolveQuestionSelection(question, selectionIndex, inputText, selectedOptionIds) {
...
export function resolveQuestionNumberShortcut(question, input) {
    if (!/^[1-9]$/.test(input)) {
        return undefined;
    }
...
export function applyQuestionResolution(queue, questionId, resolution) {
    const now = Date.now();
    const questions = (Array.isArray(queue.questions) ? queue.questions : []).map((question) => question.id === questionId
        ? {
            ...question,
        ...
        : question);
    const nextActiveQuestion = questions.find((question) => question.state === "pending");
    return {
...
export function findQuestionForFallback(queue, idOrLast) {
    if (!queue) {
        return undefined;
    }
    const questions = Array.isArray(queue.questions) ? queue.questions : [];
    if (idOrLast.trim().toLowerCase() === "last") {
        return (getActiveQuestion(queue) ??
            [...questions].reverse().find((question) => question.state === "pending") ??
            [...questions].reverse()[0]);
    }
    const normalized = idOrLast.trim().toLowerCase();
    return questions.find((question) => {
        return (question.id.toLowerCase() === normalized ||
            shortQuestionId(question.id).toLowerCase() === normalized);
    });
}
export function createPlanQuestionsForPrompt(taskPrompt) {
...
export function mapClarificationQuestionsToCards(questions) {
    return questions.map((question) => createQuestion({
        allowCustom: true,
        source: "plan",
        title: question,
        type: "free_text"
...
export function buildPromptWithQuestionAnswers(originalPrompt, questions) {
...
export function formatQuestionList(queue) {
    const questions = Array.isArray(queue?.questions) ? queue.questions : [];
    ...
    for (const question of questions) {
        const marker = question.state === "pending" && question.id === queue.activeQuestionId
            ? "*"
            : " ";
        lines.push(`${marker} ${shortQuestionId(question.id)}  [${question.state}]  ${question.title}`);
        const options = readQuestionOptions(question.options, {
            questionId: question.id,
            warn: undefined
        });
        if (question.state === "pending" && options.length > 0) {
            lines.push(`  choices: ${options.map((option, index) => `${index + 1}. ${option.label}`).join("  ")}`);
        }
        if (question.answer) {
...
export function formatQuestionDetail(question) {
    if (!question) {
...
export function shortQuestionId(value) {
    if (value.length <= 12) {
        return value;
    }
    return value.slice(-12);
}
export function isQuestionQueueState(value) {
    return restoreQuestionQueueState(value) !== null;
}
export function normalizeQuestionQueueState(value) {
...
export function restoreQuestionQueueState(value, options) {
    ...
    const questions = value.questions
        .map((question, index) => parseQuestion(question, {
        index,
        warn
    }))
    ...
    const activeQuestion = questions.find((question) => question.id === activeQuestionId && question.state === "pending") ?? questions.find((question) => question.state === "pending");
    return {
...
export function isQuestionFallbackCommand(input) {
...
export async function detectExistingFolderQuestion(input) {
    const candidate = await findExistingFolderCandidate(input.workspaceRoot, `${input.planStep.title}\n${input.planStep.description}`);
    if (!candidate) {
        return undefined;
    }
...
export function createQuestionFromToolInput(input, source, relatedTaskId) {
    ...
    const recommendationReason = readString(input.recommendationReason) ?? "best available default";
    return createQuestion({
        allowCustom: typeof input.allowCustom === "boolean"
            ? input.allowCustom
            : type === "free_text",
...
function createQuestionId(source, title) {
...
function createOptionId(value, index) {
...
function normalizeRecommendedOption(recommendedOption, options) {
    if (!recommendedOption) {
        return undefined;
    }
    const value = recommendedOption.value ?? recommendedOption.optionId ?? "";
    const matchedOption = options.find((option) => option.id === recommendedOption.optionId ||
...
function resolveSingleChoiceOption(question, input) {
    const options = readQuestionOptions(question.options, {
        questionId: question.id,
        warn: undefined
    });
...
function resolveMultiChoiceOptions(question, input) {
    const parts = input
        .split(/[,;]+|\s+and\s+/i)
        .map((part) => part.trim())
...
function createOptionAnswer(options) {
    return {
...
function parseQuestion(value, options) {
    if (!isRecord(value)) {
    ...
    if (!type) {
        options.warn?.(`Skipping saved question ${formatQuestionDebugId(value, options.index)}: unknown question type.`);
        return undefined;
    }
    const title = readString(value.title) ?? "Decision needed";
    const source = isQuestionSource(value.source) ? value.source : "tool";
    const questionId = readString(value.id) ?? createRestoredQuestionId(source, title, options.index);
    const allowCustom = typeof value.allowCustom === "boolean" ? value.allowCustom : type === "free_text";
    ...
    if ((type === "single_choice" || type === "multi_choice") &&
        ...
        options.warn?.(`Skipping saved question ${questionId}: ${type} question has no valid options.`);
        return undefined;
    }
    if (type === "confirm" &&
        questionOptions.length === 0 &&
        !allowCustom &&
        !allowSkip) {
        options.warn?.(`Skipping saved question ${questionId}: confirm question has no valid choices.`);
        return undefined;
    }
...
function normalizeQuestion(question) {
    ...
    return {
        ...
        recommendedOption: normalizeRecommendedOption(readRecommendedOption(question.recommendedOption), options),
...
function isQuestionSource(value) {
    return (value === "plan" ||
        value === "build" ||
        value === "agent" ||
        value === "tool" ||
...
async function findExistingFolderCandidate(workspaceRoot, text) {
...
function extractFolderCandidates(text) {
    const candidates = [];
    const add = (value) => {
...
function readQuestionOptions(value, options) {
    ...
    for (const [index, entry] of value.entries()) {
        if (!isRecord(entry)) {
            options.warn?.(`Skipping option ${index + 1} for saved question ${options.questionId}: not an object.`);
            continue;
        }
        const label = readString(entry.label) ?? readString(entry.value);
        if (!label) {
            options.warn?.(`Skipping option ${index + 1} for saved question ${options.questionId}: missing label.`);
            continue;
        }
...
function readRecommendedOption(value) {
    if (!isRecord(value)) {
        return undefined;
    }
...
function readQuestionAnswer(value) {
    if (!isRecord(value)) {
        return undefined;
    }
...
function readQuestionContinuationContext(value, warn) {
    if (!isRecord(value)) {
        return undefined;
    }
    if (value.type === "plan") {
...
function createRestoredQuestionId(source, title, index) {
    const hash = createHash("sha1")
...
function formatQuestionDebugId(value, index) {
    const id = readString(value.id);
    return id ? id : `${index + 1}`;
}
function readToolQuestionOptions(value) {
    if (!Array.isArray(value)) {
        return [];
    }
    const options = [];
...
function readQuestionType(value) {
