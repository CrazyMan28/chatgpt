...
export function TranscriptPanel({ entries, height, isStreaming, themeName, verbosity, width }) {
    const viewportWidth = Math.max(18, width - 4);
    const viewportHeight = Math.max(3, height - 4);
    const colors = getTuiThemeColors(themeName);
    const lines = useMemo(() => formatTranscript(entries, viewportWidth, verbosity, themeName), [entries, viewportWidth, verbosity, themeName]);
    ...
    return (_jsxs(Panel, { emphasis: isStreaming ? "strong" : "normal", footer: footer, height: height, subtitle: verbosity === "verbose" ? "verbose cards" : "focused cards", themeName: themeName, title: "Transcript", width: width, children: [_jsx(Box, { flexDirection: "column", children: visibleLines.length > 0 ? (visibleLines.map((line) => (_jsx(Text, { bold: line.bold, color: line.color, dimColor: line.dim, children: line.text }, line.key)))) : (_jsx(EmptyState, { detail: "Type a request, open /palette, or switch panels with Tab.", themeName: themeName, title: "Ready", width: Math.max(12, width - 6) })) }), lines.length === 0 ? _jsx(Text, { color: colors.muted, children: " " }) : null] }));
}
export function MessageCard({ bodyLines, borderColor, id, title, titleColor, width }) {
...
export function UserMessageCard(entry, width, colors) {
    return MessageCard({
        bodyLines: wrapMultiline(entry.content, Math.max(8, width - 4)).map((line) => ({
            color: colors.text,
            text: line
        })),
...
export function AssistantMessageCard(entry, width, colors) {
    const bodyLines = [];
    ...
    return MessageCard({
        bodyLines,
        borderColor: entry.status === "streaming" ? colors.accentSecondary : colors.borderDim,
        id: entry.id,
...
export function ToolCard(entry, width, colors, verbosity) {
    ...
    const outputSummary = detailLines.length === 0
        ? parsed.summary
...
export function ErrorCard({ bodyLines, colors, id, title, width }) {
    return MessageCard({
        bodyLines,
...
export function ProgressCard(entry, width, colors) {
    const lines = wrapPreservingLines(entry.content, Math.max(8, width - 4));
    return MessageCard({
...
function formatTranscript(entries, width, verbosity, themeName) {
    const colors = getTuiThemeColors(themeName);
    const visibleEntries = entries.filter((entry) => shouldRenderEntry(entry, verbosity));
...
function formatSystemEntry(entry, width, colors, verbosity) {
    ...
    const contentLines = wrapPreservingLines(entry.content.length > 0 ? entry.content : "...", Math.max(8, width - 4));
    const maxLines = verbosity === "quiet" && !important
        ? 1
        : verbosity === "verbose"
...
function shouldRenderEntry(entry, verbosity) {
...
function isImportantSystemEntry(content) {
    return /blocked|approval required|agent error|session error|failed|error|build stopped|rate limit/i.test(content);
}
function isLowSignalStatusEntry(content) {
    return /^(execution mode set|response style set|ui status|theme:|panel:|command palette|inspector hidden|inspector shown)/i.test(content.trim());
}
function isPlanEntry(content) {
    return /(^|\n)(plan ready|plan confirmed|project plan|task plan|todo|[-*]\s+\[[ x]\])/i.test(content);
}
function isProgressEntry(content) {
    return /build step|current step|progress|resumable build|completed step/i.test(content);
}
...
function createCardTop(title, width) {
...
function createCardBodyLine(value, width) {
    const text = truncate(value, width);
    return `│ ${text.padEnd(width, " ")} │`;
}
function createCardBottom(width) {
...
function summarizeToolResult(parsed, detailLines) {
    if (parsed.isError) {
...
function readDetailField(detail, field) {
...
function wrapPreservingLines(text, width) {
    return text
        .split("\n")
        .flatMap((line) => line.trim().length === 0 ? [""] : wrapLine(line.trimEnd(), width));
}
function wrapMultiline(text, width) {
    return text
        .split("\n")
        .flatMap((line) => line.trim().length === 0 ? [""] : wrapLine(line.trim(), width));
}
...
//# sourceMappingURL=transcript-panel.js.map