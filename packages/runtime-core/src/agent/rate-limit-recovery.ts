...
export async function runWithRateLimitRecovery<T>(input: {
  onRetry?: (input: {
    attempt: number;
    delayMs: number;
    error: unknown;
  ...
  for (let attempt = 0; attempt <= retryDelays.length; attempt += 1) {
    try {
      ...
      if (attempt >= retryDelays.length) {
        throw new Error(
          `Provider rate limit persisted after ${retryDelays.length} retries. ${readErrorMessage(
            error
          )}`
        );
      }
...
export function getRateLimitRetryDelays(): number[] {
