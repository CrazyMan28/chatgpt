...
import {
  ...
  inferMemoryFactType,
  type MemoryFactType
} from "../memory/extract-memory-facts.js";
import {
  cosineSimilarity,
  embedText
} from "../memory/text-embedding.js";
...
export class JsonMemoryStore implements MemoryStore {
  ...
  async clear(scope: MemoryScope = "workspace"): Promise<void> {
    const snapshot = await this.readSnapshot();
  ...
  private async readSnapshot(): Promise<MemorySnapshot> {
    try {
  ...
  private async writeSnapshot(snapshot: MemorySnapshot): Promise<void> {
...
function parseSnapshot(value: unknown): MemorySnapshot {
  if (!isRecord(value)) {
    return {
      memories: [],
      version: MEMORY_SCHEMA_VERSION
  ...
  return normalizeSnapshot({
    ...
    version:
      typeof value.version === "number" ? value.version : MEMORY_SCHEMA_VERSION
  });
}
...
function parseMemoryRecord(value: unknown): MemoryRecord | undefined {
  ...
  if (
    ...
    return {
      embedding: normalizeEmbedding(
        value.embedding.filter((entry) => typeof entry === "number")
      ),
      id: value.id,
  ...
  const legacySessionId =
    typeof value.sourceSessionId === "string" ? value.sourceSessionId : "legacy";
...
function normalizeEmbedding(embedding: readonly number[]): number[] {
  ...
  const magnitude = Math.sqrt(
    numbers.reduce((sum, value) => sum + value * value, 0)
  );
...
function normalizeMemoryOutputText(value: string): string {
