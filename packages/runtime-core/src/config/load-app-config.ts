...
function parseModelProvider(value: string): ModelProvider {
  ...
  throw new Error(
    `Unsupported model provider "${value}". Expected "local", "openai", "anthropic", or "mistral".`
  );
}

function loadLocalModelConfig(env: NodeJS.ProcessEnv): LocalModelConfig {
  ...
  return {
    ...
    ...(model === DEFAULT_LOCAL_MODEL
      ...
      : {
          baseUrl:
            readOptionalValue(env.CHATGPT_CODE_LOCAL_BASE_URL) ??
            DEFAULT_OLLAMA_BASE_URL
        })
  };
}

function loadOpenAIModelConfig(env: NodeJS.ProcessEnv): OpenAIModelConfig {
  const model = readOptionalValue(env.CHATGPT_CODE_MODEL);
  const apiKey = readOptionalValue(env.CHATGPT_CODE_OPENAI_API_KEY);

  if (model === undefined) {
    throw new Error(
      "CHATGPT_CODE_MODEL is required when CHATGPT_CODE_MODEL_PROVIDER=openai."
    );
  }

  if (apiKey === undefined) {
    throw new Error(
      "CHATGPT_CODE_OPENAI_API_KEY is required when CHATGPT_CODE_MODEL_PROVIDER=openai."
    );
  }
  ...
  return {
    ...
    apiKey,
    baseUrl:
      readOptionalValue(env.CHATGPT_CODE_OPENAI_BASE_URL) ??
      DEFAULT_OPENAI_BASE_URL
  };
}

function loadAnthropicModelConfig(env: NodeJS.ProcessEnv): AnthropicModelConfig {
  const model = readOptionalValue(env.CHATGPT_CODE_MODEL);
  const apiKey = readOptionalValue(env.CHATGPT_CODE_ANTHROPIC_API_KEY);

  if (model === undefined) {
    throw new Error(
      "CHATGPT_CODE_MODEL is required when CHATGPT_CODE_MODEL_PROVIDER=anthropic."
    );
  }

  if (apiKey === undefined) {
    throw new Error(
      "CHATGPT_CODE_ANTHROPIC_API_KEY is required when CHATGPT_CODE_MODEL_PROVIDER=anthropic."
    );
  }
  ...
  return {
    ...
    apiKey,
    baseUrl:
      readOptionalValue(env.CHATGPT_CODE_ANTHROPIC_BASE_URL) ??
...
function loadMistralModelConfig(env: NodeJS.ProcessEnv): MistralModelConfig {
  ...
  if (apiKey === undefined) {
    throw new Error(
      "CHATGPT_CODE_MISTRAL_API_KEY is required when CHATGPT_CODE_MODEL_PROVIDER=mistral."
    );
  }
  ...
  return {
    ...
    apiKey,
    baseUrl:
      readOptionalValue(env.CHATGPT_CODE_MISTRAL_BASE_URL) ??
...
function loadConfigFile(
  ...
  cwd: string
): { defaults: McpServerDefaults; mcpServers: McpServerConfig[] } {
  const parsedConfig = readAppConfigFile(env, cwd);

  return {
    defaults: parsedConfig.file.defaults,
    mcpServers: createEnabledMcpServerConfigs(
      parsedConfig.file,
      parsedConfig.configDirectory
    )
  };
}
