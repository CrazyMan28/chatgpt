...
export type McpFormTransition =
  | { flow: McpFormState; type: "continue" }
  | { draft: McpServerDraft; type: "complete" }
  | { message: string; type: "error" };

export function createAddMcpFormState(): McpFormState {
  return {
    draft: {
      args: [],
      command: "",
...
export function createEditMcpFormState(
...
export function getMcpFormInputState(flow: McpFormState): McpFormInputState {
    ...
    case "endpoint":
      return draft.type === "http"
        ...
        : {
            ...
            placeholder:
              flow.mode === "edit" ? draft.command || "node" : "node",
            subtitle: flow.mode === "edit" ? "Step 1 of 4" : "Step 3 of 6",
            title:
              flow.mode === "edit" ? "MCP Edit: Command" : "MCP Add: Command"
          };
    case "args":
      return {
        footer:
...
export function applyMcpFormInput(
  flow: McpFormState,
  rawInput: string
): McpFormTransition {
...
function parseMcpServerType(value: string): McpServerType | undefined {
