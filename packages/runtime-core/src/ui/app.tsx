...
export function App({
  ...
  const tryHandleNaturalSshInput = async (input: string): Promise<boolean> => {
    ...
    await executeAgentPrompt({
      ...
      toolRegistryOverride: createSingleToolRegistry(
        toolRegistry,
        createSshRunToolName(intent.agent.id)
      ),
...
function buildAgenticSshPrompt(agent: RemoteWorkerView, command: string): string {
  return [
