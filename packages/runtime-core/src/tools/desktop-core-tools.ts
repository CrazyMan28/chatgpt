import {
  ...
  type DesktopVisionAuth
} from "@chatgpt-code/desktop-control-mcp";

import type { JsonSchema, ModelConfig } from "../models/model-client.js";
...
export function createDesktopCoreToolRegistrations(
  options: DesktopCoreToolOptions
): ToolRegistration[] {
  ...
  return [
    tool("screen_screenshot", "Capture a real local desktop screenshot and return path, dimensions, timestamp, and active window metadata.", {}, () =>
      runStructured(() => controller.screenshot())
    ),
    ...
    tool("desktop_status", "Return core desktop-control status, safety mode, capabilities, active window, watcher state, and vision config.", {}, () =>
      runStructured(
        () => controller.status(),
        (status) => formatDesktopStatus(status)
      )
    ),
    ...
    tool("desktop_run", "Run a bounded desktop computer-use task loop: observe, act through safety policy, observe again, and stop when done or blocked.", {
      ...
      runStructured(
        () => controller.runTask(readString(input.task, "task")),
        (result) => formatDesktopRunResult(controller, result)
      )
    ),
    tool("desktop_stop", "Stop active desktop watchers or task loops.", {}, () =>
      runStructured(() => controller.stop())
    ),
    tool("window_list", "List visible desktop windows where the Linux session exposes them.", {}, () =>
      runStructured(() => controller.listWindows())
    ),
    tool("window_active", "Return the active desktop window where supported.", {}, () =>
      runStructured(async () => (await controller.activeWindow()) ?? null)
    ),
    ...
    tool("app_list", "List installed desktop launchers and running app process names.", {}, () =>
      runStructured(() => controller.listApps())
    ),
    ...
    tool("accessibility_type", "Focus an accessible element and type text through the safety policy.", {
      ...
      actionResult(() =>
        controller.accessibilityType(
          readString(input.elementId, "elementId"),
          readString(input.text, "text")
        )
      )
    ),
    ...
    tool("clipboard_read", "Read clipboard text through the desktop safety policy.", {}, () =>
      actionResult(() => controller.clipboardRead())
    ),
    ...
    tool("desktop_mode", "Set desktop-control safety mode.", {
      ...
      runStructured(
        () => controller.setMode(readDesktopMode(input.mode)),
        (config) => `Desktop mode set to ${config.mode}.`
      )
    ),
    tool("desktop_allow", "Add an app/window pattern to the desktop allowlist.", {
      ...
      runStructured(
        () => controller.allowApp(readString(input.app, "app")),
        (config) => `Allowed desktop apps: ${config.allowedApps.join(", ") || "all"}`
      )
    ),
    tool("desktop_block", "Add an app/window pattern to the desktop blocklist.", {
      ...
      runStructured(
        () => controller.blockApp(readString(input.app, "app")),
        (config) => `Blocked desktop apps: ${config.blockedApps.join(", ") || "none"}`
      )
    ),
    tool("desktop_allowed", "Return desktop allowlist and blocklist policy.", {}, () =>
      runStructured(
        () => controller.listAllowed(),
        (lists) =>
          [
            "Desktop App Policy",
    ...
    tool("vision_status", "Return selected desktop vision provider/model/configuration state.", {}, () =>
      runStructured(
        () => readVisionStatusWithLocalDetails(controller),
        (status) =>
          [
            "Vision Status",
    ...
    tool("vision_provider", "Set desktop vision provider.", {
      ...
      runStructured(
        () => controller.setVisionProvider(readVisionProvider(input.provider)),
        (config) => `Vision provider set to ${config.visionProvider}.`
      )
    ),
    tool("vision_model", "Set desktop vision model.", {
      ...
      runStructured(
        () => controller.setVisionModel(readString(input.model, "model")),
        (config) => `Vision model set to ${config.visionModel ?? "-"}.`
      )
    )
  ];
}

async function readVisionStatusWithLocalDetails(
  controller: DesktopController
): ReturnType<DesktopController["visionStatus"]> {
...
async function requestDesktopApproval(
  approvalManager: ApprovalManager,
...
function readDesktopVisionAuth(
  getModelConfig: (() => ModelConfig) | undefined
): DesktopVisionAuth | undefined {
  if (!getModelConfig) {
    return undefined;
  }
...
async function actionResult(
  operation: () => ReturnType<DesktopController["act"]>
): Promise<ToolExecutionResult> {
  try {
    const structuredContent = await operation();
...
function formatDesktopRunResult(
  controller: DesktopController,
  result: Awaited<ReturnType<DesktopController["runTask"]>>
): string {
  void controller;
