...
import {
  analyzeCommandPathAccess,
  classifyPathScope,
  classifyCommandSafety,
  classifyFileReadSafety,
  classifyFileWriteSafety,
...
export function createCoreToolRegistrations(
  ...
  return [
    {
      tool: {
        ...
        description:
          "Run a non-interactive command inside the currently granted scope and capture stdout, stderr, and exit status.",
    ...
    },
    {
      tool: {
        ...
        description:
          "List files and directories inside the currently granted scope with optional path and depth controls.",
    ...
    },
    {
      tool: {
        ...
        description:
          "Read a UTF-8 text file inside the currently granted scope and return its contents.",
    ...
    },
    {
      tool: {
        ...
        description:
          "Write UTF-8 text to a file inside the currently granted scope, creating parent directories when needed.",
...
const runCommandSchema: JsonSchema = {
  ...
  properties: {
    ...
    timeoutMs: {
      type: "number",
      description:
        "Optional initial idle timeout in milliseconds, capped at 300000 and automatically extended while progress is detected."
    }
  },
  required: ["command"]
};
...
const listFilesSchema: JsonSchema = {
  ...
  properties: {
    path: {
      type: "string",
      description:
        "Optional directory path relative to the current working directory. Defaults to the current working directory."
    },
    maxDepth: {
      type: "number",
  ...
  try {
    ...
    if (context.scope === "remote-host") {
      ...
      if (!context.activeRemoteHostId || !options.remoteExecutor) {
        return {
          content:
            "Remote-host scope is active, but no registered remote host executor is available.",
          isError: true
        };
      }
    ...
    const decision = await authorizeAction(options, {
      context,
      detail:
        commandPathAnalysis.primaryTargetPath
...
async function readFileTool(
  ...
  try {
    ...
    const content =
      rawContent.length > MAX_READ_BYTES
...
interface CommandExecutionResult {
  ...
  watcherStatus: "completed" | "failed" | "stalled";
}
...
async function executeCommand({
  ...
  onWatcherActivity,
  timeoutMs
}: ExecuteCommandOptions): Promise<CommandExecutionResult> {
  ...
  return {
    ...
    watcherStatus:
      finalResult.failureKind === "success"
        ? "completed"
...
async function runCommandAttempt(input: {
  ...
  args: string[];
  cwd: string;
  onWatcherActivity?: ExecuteCommandOptions["onWatcherActivity"];
  timeoutMs: number;
  ...
  return new Promise((resolve) => {
    ...
    let idleTimeoutMs = Math.min(
      Math.max(input.timeoutMs, DEFAULT_COMMAND_TIMEOUT_MS),
      MAX_COMMAND_IDLE_TIMEOUT_MS
    );
    let stdout = "";
    let stderr = "";
    let settled = false;
    let failureKind: CommandFailureKind = "success";
    ...
    const emitWatcherActivity = (metadata?: Record<string, unknown>): void => {
      const now = Date.now();
    ...
    child.stdout.on("data", (chunk: string) => {
      ...
      if (looksLikeProgress(chunk)) {
        idleTimeoutMs = Math.min(
          MAX_COMMAND_IDLE_TIMEOUT_MS,
          idleTimeoutMs + COMMAND_PROGRESS_EXTENSION_MS
        );
        emitWatcherActivity({
          idleTimeoutMs,
          stream: "stdout"
    ...
    child.once("error", (error) => {
      ...
      resolve({
        ...
        failureKind:
          /not found|ENOENT|command not found/i.test(error.message)
            ? "missing_dependency"
            : "spawn_error",
        signal: null,
...
async function createWatcher(
  ...
  input: {
    ...
    summary: string;
    target?: string;
    type: "command" | "build" | "test" | "log" | "file" | "remote";
  }
...
async function noteWatcherActivity(
  options: CoreToolOptions,
  watcherId: string | undefined,
  input?: {
    metadata?: Record<string, unknown>;
...
async function authorizeAction(
  ...
  input: {
    context: ExecutionContext;
    detail: string;
    kind: "tool" | "remote" | "mcp" | "build" | "diff";
    metadata?: Record<string, unknown>;
    resource?: string;
...
): Promise<{ approved: true; result?: undefined } | { approved: false; result: ToolExecutionResult }> {
...
function readExecutionContext(options: CoreToolOptions): ExecutionContext {
...
function classifyNonZeroExit(
  stdout: string,
  stderr: string
): Exclude<CommandFailureKind, "spawn_error" | "stalled" | "success"> {
...
function shouldRetryCommand(failureKind: CommandFailureKind): boolean {
  return (
    failureKind === "stalled" ||
...
function formatFailureKind(failureKind: CommandFailureKind): string {
  switch (failureKind) {
    case "success":
      return "success";
...
function looksLikeProgress(value: string): boolean {
  return /\b(download(?:ing)?|install(?:ing)?|extract(?:ing)?|copy(?:ing)?|compile(?:ing|d)?|build(?:ing)?|fetch(?:ing)?|transpil(?:e|ing)|bundl(?:e|ing))\b/i.test(
    value
  );
}

function classifyCommandWatcherType(
  command: string,
  ...
  if (
    /\b(test(?:ing)?|jest|vitest|playwright|cypress|pytest|cargo test|go test)\b/.test(
      normalized
    )
  ) {
    return "test";
  ...
  if (
    /\b(build(?:ing)?|compile(?:ing|d)?|tsc|webpack|rollup|vite build|next build|gradle assemble|xcodebuild)\b/.test(
      normalized
    )
  ) {
    return "build";
  ...
  if (
    /\b(tail|journalctl|logcat|watch|follow)\b/.test(normalized) ||
...
function isMissingFileError(
  error: unknown
): error is NodeJS.ErrnoException & { path?: string } {
  return (
    typeof error === "object" &&
    error !== null &&
    "code" in error &&
...
function readAttemptCount(value: string): number {
