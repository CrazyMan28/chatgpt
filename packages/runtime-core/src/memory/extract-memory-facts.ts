...
export function extractTypedMemoryFacts(
  message: string
): ExtractedMemoryFact[] {
  const facts = splitIntoSentences(message)
    .map((sentence) => extractFact(sentence))
    .filter((fact): fact is ExtractedMemoryFact => fact !== undefined);

  const uniqueFacts = new Map<string, ExtractedMemoryFact>();
...
function extractFact(sentence: string): ExtractedMemoryFact | undefined {
...
export function inferMemoryFactType(value: string): MemoryFactType {
  const normalized = value.trim().toLowerCase();

  if (
    /\b(my name is|call me|i am|i'm|we are)\b/.test(normalized) &&
  ...
  if (
    /\b(i prefer|we prefer|i like|we like|favorite|favourite|prefer to|prefer using)\b/.test(
      normalized
    )
  ) {
    return "preference";
  ...
  if (
    /\b(project|building|working on|roadmap|release|feature|product)\b/.test(
      normalized
    )
  ) {
    return "project";
  ...
  if (
    /\b(i use|we use|always|usually|workflow|process|routine|stack|tooling)\b/.test(
      normalized
    )
  ) {
    return "workflow";
