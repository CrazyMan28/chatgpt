...
import type { AgentTurnEvent } from "../agent/run-agent-turn.js";
import {
  AgentControlError,
  runAgentTurn
...
export function detectFleetDelegationIntent(prompt: string): boolean {
  return /\b(sub-?agent|delegate(?:\s+this)?|fleet)\b/i.test(prompt);
}

export function inferFleetRoleFromPrompt(prompt: string): WorkerSpec["role"] {
...
class FleetManagerImpl implements FleetManager {
  private readonly agents = new Map<string, FleetAgentRecord>();
  private readonly listeners = new Set<
    (agents: readonly FleetAgentRecord[]) => void
  >();
  private readonly locks = new Map<string, TaskLock>();
  private readonly runtimes = new Map<string, AgentRuntimeState>();
  private enabled = false;

  constructor(private readonly dependencies: FleetManagerDependencies) {
  ...
  async assign(input: {
    ...
    const maxConcurrentAgents = Math.max(
      1,
      this.dependencies.maxConcurrentAgents ?? DEFAULT_MAX_AGENTS
    );

    if (activeAgents.length >= maxConcurrentAgents) {
      throw new Error(
        `Fleet is limited to ${maxConcurrentAgents} concurrent sub-agents.`
      );
    }
    ...
    for (const lockKey of lockKeys) {
      ...
      if (existing) {
        throw new Error(
          `Fleet task conflict: "${lockKey}" is already locked by ${existing.ownerId}.`
        );
      }
    }

    const workerSession = await this.dependencies.sessionStore.createSession(
      createInitialSessionState()
    );
    const now = Date.now();
    const scopedToolRegistry = createExecutionModeToolRegistry(
      this.dependencies.toolRegistry,
      input.mode
    );
    const agent: FleetAgentRecord = {
  ...
  async pauseAgent(id: string): Promise<FleetAgentRecord | undefined> {
  ...
  async resumeAgent(id: string): Promise<FleetAgentRecord | undefined> {
    const agent = this.agents.get(id);
  ...
  async stopAgent(id: string): Promise<FleetAgentRecord | undefined> {
    const agent = this.agents.get(id);
  ...
  async restartAgent(id: string): Promise<FleetAgentRecord | undefined> {
    const agent = this.agents.get(id);
  ...
  private async runAgent(
    ...
    try {
      const session =
        (await this.dependencies.sessionStore.loadSession(
          agent.workerSessionId ?? agent.parentSessionId
        )) ??
          ...
          runAgentTurn({
            ...
            onEvent: async (event: AgentTurnEvent) => {
              ...
              this.updateAgent(agent.id, {
                ...
                currentCwd: process.cwd(),
                currentScope: "workspace",
                recentOutput: appendRecentOutput(
                  this.agents.get(agent.id)?.recentOutput,
                  describeFleetEvent(event)
                ),
            ...
            toolRegistry: createExecutionModeToolRegistry(
              this.dependencies.toolRegistry,
              mode
            )
          })
  ...
  private acquireLocks(agent: FleetAgentRecord): void {
  ...
  private releaseLocks(agentId: string): void {
  ...
  private updateAgent(
    agentId: string,
    patch: Partial<Omit<FleetAgentRecord, "id" | "createdAt" | "parentSessionId" | "role" | "task">>
  ): void {
    ...
    const nextAgent: FleetAgentRecord = {
      ...current,
      ...patch,
      updatedAt: Date.now()
    };
  ...
  private async saveUpdatedAgent(
    agentId: string,
    patch: Partial<Omit<FleetAgentRecord, "id" | "createdAt" | "parentSessionId" | "role" | "task">>
  ): Promise<FleetAgentRecord | undefined> {
    const current = this.agents.get(agentId);
  ...
  private async persistAgent(
    agent: FleetAgentRecord | undefined
  ): Promise<void> {
...
function createFleetAgentId(): string {
  return `agent-${Date.now().toString(36)}-${Math.random().toString(16).slice(2, 8)}`;
}

function createRuntimeState(agent: FleetAgentRecord): AgentRuntimeState {
  return {
    isRunning: false,
...
function normalizeRecoveredAgent(agent: FleetAgentRecord): FleetAgentRecord {
...
function buildFleetPrompt(role: WorkerSpec["role"], task: string): string {
  return [
...
function deriveAgentMode(agent?: FleetAgentRecord): AgentMode {
...
function deriveResponseMode(agent?: FleetAgentRecord): ResponseMode {
  const mode = agent?.contextSnapshot?.responseMode;
  return typeof mode === "string" && isResponseMode(mode) ? mode : "normal";
}

function detectLockKeys(task: string): string[] {
...
function extractChangedFiles(
  event: Extract<AgentTurnEvent, { type: "tool_finished" }>
): string[] {
...
function describeFleetSummary(event: AgentTurnEvent): string {
  switch (event.type) {
    case "status":
...
function describeFleetEvent(event: AgentTurnEvent): string {
  switch (event.type) {
    case "status":
