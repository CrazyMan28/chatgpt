  ...
  try {
    ...
    if (request.method === "POST" && url.pathname === "/tasks/start") {
      ...
      respondJson(
        ...
        await runtime.startTask({
          mode: optionalString(body.mode) as AgentMode | undefined,
          prompt: optionalString(body.prompt),
          sessionId: optionalString(body.sessionId),
          style: optionalString(body.style) as "normal" | "plan" | "ultra" | undefined,
    ...
    if (request.method === "POST" && url.pathname === "/chat") {
      ...
      respondJson(
        ...
        await runtime.runPrompt({
          mode: readString(body.mode, "normal") as AgentMode,
          prompt: readString(body.prompt),
          sessionId: readString(body.sessionId),
          style: readString(body.style, "normal") as "normal" | "plan" | "ultra"
        })
      );
      return;
    }
    ...
    if (request.method === "GET" && /^\/sessions\/[^/]+$/.test(url.pathname)) {
      respondJson(
        response,
        200,
        await runtime.getSession(decodeURIComponent(url.pathname.split("/")[2] ?? ""))
      );
      return;
    }
    ...
    if (
      ...
      respondJson(
        ...
        await runtime.attachSession({
          ...
          clientType: readString(body.clientType, "web") as "tui" | "web" | "mobile" | "remote" | "desktop" | "daemon",
    ...
    if (request.method === "POST" && url.pathname === "/login") {
      ...
      respondJson(
        ...
        await runtime.login({
          ...
          provider: readString(body.provider) as "local" | "openai" | "anthropic" | "mistral"
        })
      );
      return;
    }
    ...
    if (request.method === "POST" && url.pathname === "/fleet/assign") {
      ...
      respondJson(
        response,
        200,
        await runtime.assignFleetTask({
          mode: readString(body.mode, "normal") as AgentMode,
          parentSessionId: readString(body.parentSessionId),
          role: readString(body.role, "general") as "general" | "frontend" | "backend" | "tester" | "devops" | "docs" | "blender",
          style: readString(body.style, "normal") as "normal" | "plan" | "ultra",
    ...
    if (request.method === "GET" && url.pathname === "/memory/search") {
      respondJson(
        response,
        200,
        await runtime.searchMemory(url.searchParams.get("q") ?? "")
      );
      return;
    }
    ...
    if (request.method === "GET" && url.pathname.startsWith("/timeline/filter/")) {
      ...
      respondJson(
        response,
        200,
        await runtime.timelineStore.filterByType(type as never)
      );
      return;
    }
    ...
    if (request.method === "POST" && url.pathname === "/mcp/sets/save") {
      ...
      respondJson(
        response,
        200,
        await runtime.saveMcpSet({
          description: optionalString(body.description),
          name: readString(body.name),
          projectId: optionalString(body.projectId),
          serverNames: Array.isArray(body.serverNames)
            ? body.serverNames.filter((value): value is string => typeof value === "string")
            : []
        })
      );
      return;
    ...
    if (request.method === "POST" && url.pathname === "/mcp/sets/assign") {
      ...
      respondJson(
        response,
        200,
        await runtime.assignProjectMcpSet(
          readString(body.projectId),
          readString(body.setId)
        )
      );
      return;
    }
    ...
    if (request.method === "POST" && url.pathname === "/auth/pairings") {
      ...
      respondJson(
        response,
        200,
        await runtime.createDevicePairing({
          expiresInMinutes:
    ...
    if (request.method === "POST" && url.pathname === "/auth/sessions") {
      ...
      respondJson(
        response,
        200,
        await runtime.createAuthSession({
          clientType: readString(body.clientType, "web") as "tui" | "web" | "mobile" | "remote" | "desktop" | "daemon",
    ...
    if (request.method === "POST" && url.pathname === "/workers") {
      ...
      respondJson(
        response,
        200,
        await runtime.remoteWorkers.add({
          capabilities: Array.isArray(body.capabilities)
            ? body.capabilities.filter((value): value is string => typeof value === "string")
            : [],
          connectionType: readString(body.connectionType, "ssh") as "local" | "ssh" | "container" | "subprocess",
          host: readString(body.host),
          id: readString(body.id),
          name: readString(body.name),
          port: typeof body.port === "number" ? body.port : undefined,
          role: readString(body.role, "general") as "general" | "frontend" | "backend" | "tester" | "devops" | "docs" | "blender",
          status: readString(body.status, "offline") as "idle" | "running" | "waiting" | "degraded" | "offline",
    ...
    if (request.method === "POST" && url.pathname.startsWith("/workers/connect/")) {
      respondJson(
        response,
        200,
        await runtime.remoteWorkers.connect(url.pathname.slice("/workers/connect/".length))
      );
      return;
    }
    ...
    if (request.method === "POST" && url.pathname.startsWith("/workers/disconnect/")) {
      respondJson(
        response,
        200,
        await runtime.remoteWorkers.disconnect(url.pathname.slice("/workers/disconnect/".length))
      );
      return;
    }
    ...
    if (request.method === "POST" && url.pathname.startsWith("/workers/test/")) {
      respondJson(
        response,
        200,
        await runtime.remoteWorkers.testConnection(url.pathname.slice("/workers/test/".length))
      );
      return;
    }
