    ...
    return {
        ...
        async runCommand(id, command, args = []) {
            ...
            if (agent.connectionType === "ssh") {
                const invocation = buildSshInvocation(agent, buildRemoteCommand(agent, command, args));
...
async function runRemoteProbe(agent) {
    if (agent.connectionType === "ssh") {
        const invocation = buildSshInvocation(agent, buildRemoteCommand(agent, "printf", ["connected"]));
...
function buildSshInvocation(agent, remoteCommand) {
    const sshArgs = [
        "-o",
        "ConnectTimeout=10",
        "-o",
