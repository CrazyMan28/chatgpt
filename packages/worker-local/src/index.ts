import {
  ...
  createExecutionModeToolRegistry,
  createSystemEntry,
  createUserEntry,
...
export function createLocalWorker(
  dependencies: LocalWorkerDependencies
): LocalWorker {
  return {
    async runPrompt(input, options) {
      const session =
        (await dependencies.sessionStore.loadSession(input.sessionId)) ??
      ...
      const result = await runWithRateLimitRecovery({
        onRetry: async ({ attempt, delayMs, error }) => {
          turnTranscript = appendTranscriptEntry(
            [...baseTranscript],
            createSystemEntry(
              `Provider rate limited this worker turn. Retrying in ${delayMs}ms (attempt ${attempt}/3).`,
              true
            )
          );
