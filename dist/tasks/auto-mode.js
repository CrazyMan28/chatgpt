...
export const AUTO_MODE_PROMPT_PLACEHOLDER = "Autonomous project improvement mode";
export const AUTO_MODE_MAX_AGENT_STEPS = 100;
export const AUTO_MODE_MAX_TOOL_ACTIONS = 100;
export function createEmptyAutoModeState() {
    return {
        completedTasks: [],
        previousActions: []
    };
...
export function buildAutoModeCyclePrompt(input) {
    ...
    const completedTasks = autoState.completedTasks.slice(0, AUTO_MODE_MAX_COMPLETED_TASKS);
...
export function createAutoModeToolRegistry(toolRegistry, options) {
    if (!toolRegistry) {
        return undefined;
    }
...
export function collectChangedFilesFromToolCall(toolCall) {
...
export function updateAutoModeState(currentState, input) {
...
function createAutoModeActionRecord(input) {
    return {
        changedFiles: dedupeStrings(input.changedFiles.map((file) => file.trim()).filter((file) => file.length > 0)).slice(0, 12),
        cycleNumber: input.cycleNumber,
        summary: extractAutoModeSummary(input.assistantContent),
        timestamp: input.timestamp
    };
}
function extractAutoModeSummary(content) {
...
function formatActionRecord(entry) {
    const changedFiles = entry.changedFiles.length > 0 ? entry.changedFiles.join(", ") : "no file changes recorded";
