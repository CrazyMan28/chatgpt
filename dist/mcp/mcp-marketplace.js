...
export function getMarketplaceEntry(name) {
    return MARKETPLACE_ENTRIES.find((entry) => entry.name.toLowerCase() === name.trim().toLowerCase());
}
export function formatMarketplaceList(entries) {
    if (entries.length === 0) {
        return "No marketplace entries are available.";
    }
    const lines = ["MCP marketplace", ""];
    for (const entry of entries) {
        lines.push(`${entry.name.padEnd(14, " ")} ${entry.draft.type.padEnd(8, " ")} ${entry.description}`);
    }
    lines.push("");
    lines.push("Use /mcp info <name> for details or /mcp install <name> to install.");
    return lines.join("\n");
}
export function formatMarketplaceInfo(entry) {
    const lines = [
        `${entry.name}`,
        entry.description,
        "",
    ...
    if (entry.prompts && entry.prompts.length > 0) {
        lines.push("");
        lines.push(`Prompts: ${entry.prompts.map((prompt) => prompt.label).join(", ")}`);
    }
    return lines.join("\n");
}
